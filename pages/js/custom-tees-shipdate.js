/**
 * custom-tees-shipdate.js — PURE ship-date promise engine for Custom T-Shirts.
 *
 * Cloned from 3-day-tees-shipdate.js (2026-06-10). Two promise modes:
 *   promise(now)         — the 3-Day RUSH promise (9 AM cutoff → +3 business
 *                          days), used when the rush toggle is on.
 *   standardPromise(now) — the STANDARD promise: production clock starts the
 *                          next business day (no same-day cutoff), ships in
 *                          7–10 business days; the BINDING stamped date is
 *                          the END of the window (under-promise, over-deliver).
 *
 * Loaded by the page (banner + Pay button + review) AND require()d by
 * server.js, which stamps the BINDING promise date on the quote session at
 * checkout time — so the customer's screen and the order record can never
 * disagree.
 *
...
(function (global) {
    ...
    const CUTOFF_HOUR = 9; // 9 AM Pacific (rush mode only)
    const PROMISE_BUSINESS_DAYS = 3;            // rush promise
    const STANDARD_MIN_BUSINESS_DAYS = 7;       // standard window start
    const STANDARD_MAX_BUSINESS_DAYS = 10;      // standard window end (binding)
    ...
    /**
     * STANDARD-turnaround promise: no same-day cutoff — the production clock
     * starts the next business day whatever time the order lands; the shop
     * window is +7 to +10 business days from there. shipDateIso/Long/Short
     * carry the END of the window so server stamping code works identically
     * for both modes (the stamped date is the binding outer promise).
     */
    function standardPromise(now) {
        const p = pacificNow(now);
        const today = { y: p.y, m: p.m, d: p.d };
        const orderDay = nextBusinessDay(addDays(today, 1));
        const shipStartDay = addBusinessDays(orderDay, STANDARD_MIN_BUSINESS_DAYS);
        const shipEndDay = addBusinessDays(orderDay, STANDARD_MAX_BUSINESS_DAYS);
        return {
            mode: 'standard',
            orderDay: orderDay,
            shipStartDay: shipStartDay,
            shipEndDay: shipEndDay,
            shipStartIso: iso(shipStartDay),
            shipEndIso: iso(shipEndDay),
            shipStartShort: formatShort(shipStartDay),
            shipEndShort: formatShort(shipEndDay),
            rangeLabel: `${formatShort(shipStartDay)} – ${formatShort(shipEndDay)}`,
            // Binding stamped date = END of the window (same keys as promise()).
            shipDateIso: iso(shipEndDay),
            shipDateLong: formatLong(shipEndDay),
            shipDateShort: formatShort(shipEndDay),
            tzAbbr: pacificTzAbbr(now),
        };
    }
    ...
    const TDTShipDate = {
        ...
        STANDARD_MIN_BUSINESS_DAYS: STANDARD_MIN_BUSINESS_DAYS,
        STANDARD_MAX_BUSINESS_DAYS: STANDARD_MAX_BUSINESS_DAYS,
