...
window.OrderFormPricing = (function () {
  ...
  async function priceForm({ rows, formCtx, addOns }) {
    ...
    try {
      ...
      result.taxRateIsDefault = !!taxCtx.rateIsDefault; // true when the WA 10.2% Milton fallback drove the rate (no destination lookup) — UI surfaces a verify-rate warning
      result.taxLabel       = taxCtx.label;       // UI label: "WA Sales Tax (10.2%)" / "Out of state — no tax"
