...
- **Quote Prefixes**: `DTG · RICH · EMB · EMBC · CEMB · LT · PATCH · SPC · SSC · WEB · OF · CAP · SAM · STK` (CAP = Custom Hats storefront, SAM = paid sample orders, OF = retired Order Form 2026-07-11 — all three read-only in Quote Mgmt, never builder-editable. STK = stickers; since 2026-07-29 it is minted ONLY by the public `/custom-stickers` page via `POST /api/public/sticker-quote`, so STK quotes carry no sales rep — the staff sticker tool that used to mint them was retired.)
