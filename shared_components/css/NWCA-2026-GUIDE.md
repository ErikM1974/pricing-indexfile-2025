...
- Quote builders: `quote-workspace.css` owns common layout; `quote-embroidery.css`, `quote-screenprint.css`, `quote-dtf.css` and `quote-dtg.css` own method-specific arrangement. Shared artwork and safety recommendations retain their own small owners. Keep pricing and saved-data behavior separate from presentation edits. Builder markup carries no `style=""` attributes, static or generated: markup that starts hidden but is toggled through `el.style.display` uses `hidden data-qb-hidden` (quote-builder-utils.js moves that state to CSSOM before the builders run); runtime templates use classes in the owning sheet, colour swatches use `getSwatchAttrs()` plus `qbPaintSwatches()`, and other data-driven values are set through CSSOM.
