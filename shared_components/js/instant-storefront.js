...
(() => {
    ...
    for (const dialog of page.querySelectorAll('dialog')) {
        dialog.addEventListener('keydown', event => {
            ...
            const controls = [...dialog.querySelectorAll('a[href],button:not(:disabled),input:not(:disabled),select:not(:disabled),textarea:not(:disabled),summary')]
    ...
    window.addEventListener('beforeprint', () => {
        ...
        disclosures.forEach(item => { item.node.open = true; });
    });
    window.addEventListener('afterprint', () => {
        disclosures?.forEach(item => { item.node.open = item.open; });
        disclosures = null;
    });
})();
