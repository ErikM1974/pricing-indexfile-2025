...
| `builders/emb/index.js` | **ESM entry point (roadmap 0.1/0.4)** — the ONE sanctioned window re-export surface for the EMB decomposition (eslint-enforced); runs before DOMContentLoaded. Extracted so far: `pricing.js`. Map → memory/emb-decomposition-plan.md |
