...
| `dtg-canonical-pricing.js` | **THE DTG formula (Batch 6, 2026-07-09)** — UMD, vendored BYTE-IDENTICAL from `caspio-pricing-proxy/lib/dtg-canonical-pricing.js` (equality CI-enforced both repos). `window.DTGCanonicalPricing`; dtg-pricing-service.js + dtg-inline-form.js delegate ALL math here. NEVER edit this copy — edit the proxy file and re-copy |
...
| `builders/dtg/index.js` | **DTG bridge shell (2026-07-08)** — window bridges for the shared error surfaces only (DTG stays inline-form architecture; NOT decomposed). DTG page now flows through the dist rewrite like the trio |
