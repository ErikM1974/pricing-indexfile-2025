...
| `builders/shared/modal-dialog.js` | **Native modal dialogs (2026-09-17)** — `showModalDialog(dialog, {initialFocus, onDismiss})` returns `close(...focusTargets)`. The `<dialog>` is the dimmed full-screen layer (page overlay class + `qb-overlay-dialog`); Tab wraps inside the newest dialog, Escape/backdrop are consumed once (page Escape handlers stay quiet), closing focuses the first target that takes focus, then the opener. Toasts render under the modal layer, so show dialog errors inside the dialog. Used by DTG confirms + EMB goods/names/manual-item dialogs |
...
| `builders/dtg/fees.js` | **Art charges (2026-08-06)** — THE authority for GRT-50 (art setup / logo mockup) + GRT-75 (graphic design/hr): rep-entered COUNTS × the live Caspio Service_Codes rate. `effectiveShipFee()` pattern — screen/PDF/save/push all read `artFeeTotals()`, never state or the DOM. `artFeeAddOns()` feeds `body.addOns`, which server.js reprices from Service_Codes (FLAT → qty × SellPrice) into ShopWorks LinesOE, so quoted == billed. 🔴 Counts, NOT a typed dollar amount — a typed amount has no push representation and would be quoted but never billed |
...
- **express-eligibility.js** — buy-online bridge: express-storefront eligibility from the storefronts' own whitelists; `ExpressEligibility.get()` → `{tees, caps, linkFor(style,color)}`. Enhancement-only (failure = no badge). Consumers: catalog cards/QV, PDP lane, homepage shelf. (2026-08-25)
| `brands-registry.js` | **THE brand source of truth (2026-07-25)** — `window.NWCA_BRANDS`: `FEATURED` (15 brands with a `/custom-<brand>` landing page: name/href/logo), `CATALOG_BRANDS` (46 verified `/api/all-brands` spellings), `LANDING_ALIASES`, plus DERIVED `LANDING_PAGES` + `SEARCH_KEYWORDS` + `PRIORITY_ORDER` + `landingPageFor(name)`. Replaced four hand-maintained copies (brands-flyout, product-search-service, brands.js, drawer HTML) that had drifted into live bugs — 5 brands had landing pages but no search keyword, and 7 keywords named brands the catalog lacks (`Bella+Canvas` vs `Bella + Canvas` → **0 products for a top seller**). Search keywords are GENERATED from catalog spellings, so a keyword can never name a missing brand; keywords are ≥3 chars and ordered longest-first because the consumer matches with `includes()` (2-letter keys turned "pants"/"PC61" into brand filters). **Script-tag it BEFORE its consumers.** Dual browser/Node; locked by tests/unit/brands-registry.test.js (incl. an assertion that the static drawer HTML still matches) |
...
| `dash-tabs.js` | **Shared tab router for dashboard pages (2026-07-26).** `DashTabs.create({tablist, tabSelector, hashKey, defaultTab, activateDelay, focusPanelOnDeepLink, onActivate, onBeforeSwitch})` → `{activate, current, isMounted, resetMounted, setBadge, ids}`. Full ARIA APG tablist: roving tabindex, arrows wrap, Home/End, and `1`–`9` by position — **all key handling is scoped to the tablist**, so there is no global handler that could hijack typing in a form field. Syncs `#tab=<id>` with `replaceState` (preserving other hash keys, so `&focus=` deep links survive), toggles panels with the `hidden` attribute, and tracks first-activation per tab so the host page can lazy-load data. **`onActivate` is debounced (`activateDelay`, default 250ms) while the visual switch stays synchronous** — with automatic activation, arrowing across five tabs would otherwise fire five tabs' worth of network. Warns loudly at runtime if the host page is missing `[hidden]{display:none!important}` (an author `display` rule silently defeats panel hiding — LESSONS recurrence #5). **Use this instead of hand-rolling**: seven pages had each written their own before this existed (sanmar-payables, unqualified-leads, quote-management, customer-portal-admin, finished-photos, finished-photos-library, ae-dashboard), most missing keyboard support or `aria-selected`. Requires no CSS of its own; style off `[aria-selected="true"]`. |
...
| File | Purpose |
|------|---------|
| `decoration-methods.js` | Decoration method eligibility (NEW 2026-06-11) — `/api/decoration-methods` rules+overrides, sessionStorage 1h cache. `eligibleFor(product)` → EMB/SCP/DTF bools + DTG 'yes'/'warn'/'no' cotton gate; `categoriesFor(method)` feeds the /catalog Decoration filter. API down → embroidery-only fallback (`source:'fallback'` — caller MUST show a visible warning). Used by product.html + /catalog. Staff Quick Quote uses the rules only as notes (Erik 2026-09-16): screen print and DTF are never limited there; DTG/embroidery outside a rule price with a "check the garment" note. |
| `headwear-classifier.js` | Cap / flat headwear / garment — THE rule for every price surface (EMB/SCP builders, DTF search, product page, embroidery calculators, Quick Quote; `isCap` → cap pricing, else garment). Erik 2026-09-16: soft headwear (beanies, headbands, gaiters, face masks, skull/scrub caps, Caps-category hood) is flat in any category, visors are caps, bandanas are garments. `HeadwearClassifier.classify(row)` → `{kind, isCap, isFlat, confident, reason}` from CATEGORY/SUBCATEGORY, title (minus "cap sleeve") and, only when the category is blank, cap shapes (trucker, 5–7 panel, bucket) and description anatomy — unless the title has a garment word (jacket, bag, tee…), which makes it an unconfident garment. Richardson caps have no category; Richardson/New Era also sell apparel, so brand and style prefixes are not used. Last resort: a bare 2–3 digit style is an unconfident cap (as the EMB builder assumes). `confident:false` = callers keep the rep's choice (Quick Quote still treats an unconfident cap as a cap in Quick Price). `isFlat` (beanies, headbands, gaiters…) is priced as flat garment embroidery on every surface; the cross-surface lock is `tests/unit/headwear-surfaces-parity.test.js`. |
| `product-category-filter.js` | Legacy cap/flat keyword lists. No longer a pricing authority (2026-09-16): only the DTF builder search uses it, to keep hiding exactly the caps it hid before. Use `headwear-classifier.js` for any cap/garment decision. |
...
| File | Purpose |
|------|---------|
| `dst-quote-math.js` | **Stitch-file quoting math (2026-08-05)** — pure UMD (no DOM), used by `/calculators/embroidery-contract/` and require()d by `tests/unit/dst-quote-math.test.js` (35 cases). Five jobs: `assessRisk()` advisory production flags (satin >12.7 mm, density hotspots, trims per 1K, colour changes), `estimateMachineHours()` single-head run time, `combineLines()` multi-location totals, `estimateMargin()` staff-only cost/margin, and `fingerprint()` file identity (SHA-256, FNV-1a fallback off a secure context). 🔑 **It never converts stitches→dollars** — `combineLines` takes ALREADY-priced lines so the calculator's `computeUnit()` stays the only pricing path (rule 9). 🔑 **LTM is ONE fee per ORDER at the highest applicable rate** (Erik 2026-08-04): a left-chest + full-back combo under 24 pcs is $100, never $150. 🔴 **`estimateMargin` takes its rates as an ARGUMENT and returns null without them** — the contract calculator is a PUBLIC page, so cost rates must arrive from the requireStaff `/api/contract-embroidery/cost-model` and may never be hardcoded into any bundle this page loads. |
...
| `staff-dashboard/controllers/tool-grid-controller.js` | Persists open/closed state for any `details[data-collapse-key]` (Pride Wall, the "More pricing" / "Go straight to a builder" folds) into the `widgetCollapse` store slot — **add that one attribute to make a new collapsible widget remember itself**, no JS needed. Its 2026-07-29 count-badge job is dormant since the Quick Access grid retired with the Workspaces layout (2026-09-03). |
| `staff-dashboard/controllers/nav-access-controller.js` | Role-aware gating. Reveals `[data-requires-role]` blocks (the Admin tab + panel, the per-rep My Accounts tiles, House Accounts) only when `/api/crm-session/me` reports the permission; **removes** the node otherwise so it also leaves the Ctrl+K registry and the generated Everything tab (both harvested from the DOM). Fails closed. **Add `data-requires-role` + `hidden` to gate a new block — but always add the matching server-side rule too; this is UX, not security** (real gate: `lib/page-access.js`). The `hidden` attribute is THIS controller's signal — nothing else on the dashboard may set or clear it on a gated node. |
| `staff-dashboard/controllers/workspace-controller.js` | **Workspaces (2026-09-03).** The role-based tabs (Sales · Production · Art · Office · Company · Everything · Admin). Default tab from the session's permissions — `admin` is checked FIRST because `permissionsFromRole` fans an admin out to accountant/house/rep tags — then remembered per browser and per email (`nwca-dash:workspace`); `#ws=<tab>` in the hash wins. Panels hide with the `is-on` class, never `hidden`. The Everything tab is GENERATED from the other panels (union of `a.ws-link`, deduped by href, grouped by each card's `data-cat`), so **to add a tool, add one `a.ws-link` row/tile to the right tab(s) and nothing else** — Everything, Ctrl+K and My Stuff pick it up from the DOM. Locked by `tests/unit/staff-dashboard-workspaces.test.js` (every 2026-09-03 href still present, links resolve, no duplicates per tab) and `tests/ui/test-workspaces.html`. |
