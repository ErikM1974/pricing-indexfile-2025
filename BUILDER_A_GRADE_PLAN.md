...
> **SESSION 3 (final EMB session):** 3.2 remainder — `showServicePricingReview` (~622, spr-modal), `_recalculatePricingImpl` (~354) + `updatePricingDisplay` (~353) (pricing-sync — the live money engine, wants fresh context), `renderImportPreview` (264) · 3.6 suppression burn-down (file-level @ts-nocheck where checkJs passes).
