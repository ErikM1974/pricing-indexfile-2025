...
| `/dashboards/art-hub-steve.html` | Art hub — Steve's personalized view. ⚠️ **Express Submit Form RETIRED 2026-08-01** — its Caspio DataPage embed (`a0e1500073092d827fb74d968d9d`) wrote STRAIGHT into ArtRequests, skipping `POST /api/artrequests` where both submission notifications live, so those requests reached Steve's queue with nobody notified. Tab now shows a signpost to `/ae-dashboard.html#submit`. **Do NOT re-add a submission DataPage** — `tests/unit/art-submit-identity.test.js` fails the build if one appears here or on ae-dashboard. | art-hub.css, art-hub-steve.js | ✅ Active |
