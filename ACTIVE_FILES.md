...
| `/pages/forms/nwca-form-styles.js` | **NEW (2026-07-11)** SanMar style lookup for form twins — per-row type-ahead via `/api/stylesearch`; pick fills Description + swaps Color to that style's real color list via `/api/product-colors` (COLOR_NAME shown, CATALOG_COLOR remembered on the row for a future ShopWorks push); "type manually" always available | APP_CONFIG, /api/stylesearch, /api/product-colors | ✅ Active |
