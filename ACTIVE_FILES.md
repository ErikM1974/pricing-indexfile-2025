...
| `/shared_components/js/policies/policy-ai-assist.js` | **NEW** AI Assist modal for the TipTap editor — action picker (Generate / Polish / Expand / Summarize / FAQ / Translate to Spanish), SSE stream consumer with live token rendering, Insert or Replace into editor | `/api/policies/ai-assist`, DOMPurify | ✅ Active |
| `/shared_components/lib/policy-ai-prompt.js` | **NEW** Frozen system prompt for the AI Assist endpoint — NWCA voice/style guidance, action definitions, output-format rules. CommonJS module require'd by server.js. Cached on every Claude API request via cache_control: ephemeral. | (none) | ✅ Active |
