...
| `/dashboards/js/payroll.js` | Payroll controller — leave balances, per-period hours, and the 3-step upload (read → reconcile → save). Sends only a job id on commit so figures can't be edited in transit. | /api/crm-proxy/payroll | ✅ Active |
| `/dashboards/css/payroll.css` | Payroll dashboard styles (NWCA green, no inline; negative balances flagged) | - | ✅ Active |
