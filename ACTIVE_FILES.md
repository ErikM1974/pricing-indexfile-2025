...
| `shared_components/js/box-url.js` | **Normalises Box READ urls onto this origin** so the app's session-gated forwarder (`server.js` `boxForward`) authorises them via the SAML cookie — which rides along automatically even on `<img>` requests. Needed because absolute proxy urls were already PERSISTED in Caspio (transfer/mockup `Thumbnail_URL`/`File_URL`), so those rows are normalised at render time instead of being rewritten. Rewrites READ routes only (thumbnail/download/art-folders/mockup-folders/folder-files/search/shared-image); the four Box WRITE routes still go straight to the proxy. Idempotent + null-safe. Locked by `tests/unit/box-url.test.js` | pages/supacolor-job-detail.html, pages/transfer-detail.html | ✅ Active (NEW 2026-08-05) |
