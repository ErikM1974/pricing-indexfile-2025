...
| `/product/js/pdp-configurator.js` | 3-question decoration configurator (Phase 1 quote-cart): qty stepper, placement chips (garment/cap) FILTERED to placements an eligible method can actually price (2026-08-18 — embroidery-only products no longer show dead Center front/Full front/Center back chips), SCP ink stepper, per-method priced chips + total/LTM-honesty/nudge via QuoteCartEngine.singleItemPreview (zero local price math), location-aware tier matrix OPEN BY DEFAULT, per-chip visible error states; getSelection() exposes status/engineMethod/sizes for the Add-to-Quote flow | QuoteCartEngine + EmbroideryPricingCalculator + DTFConfig + EMB/CAP/DTG/SCP/DTF PricingService globals | ✅ Active (NEW 2026-06-11) |
...
| `/tests/dom/` (setup, global-setup, adapter-contract.test, dtf-child-rows.test, service-code-fallback-visibility.test, pdp-placement-chips.test) | **NEW (2026-07-08, roadmap 1.14)** jsdom jest project. `pdp-placement-chips` (2026-08-18) drives the REAL product.html configurator markup: locks the placement chip row to placements an eligible method can price, and the price table rendering open; mutation-tested both ways. `service-code-fallback-visibility` (2026-08-17) locks `getServicePrice()` warning on BOTH fallback paths — missing row + unparseable SellPrice — and staying silent before the fetch resolves; mutation-tested. jsdom jest project: all 3 adapters boot QuoteBuilderBase in a real DOM (dropped contract method fails loudly); nudge tiers locked + cross-checked vs the utils map (all 4 builders); DTF childRows Map semantics off the real prototype. global-setup pre-bundles (esbuild can't run inside jsdom) | jest jsdom project, esbuild | ✅ Active |
