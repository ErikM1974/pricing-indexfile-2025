...
| `/dashboards/js/purchasing-portal.js` | Purchasing Portal controller — same-origin `/api/crm-proxy/purchasing-portal` fetch (proxy joins JotForm × PurchaseOrders × ORDER_ODBC), flattens to one row per work order, client search + requester/status filters; esc() everywhere, visible errors | DashPage, crm-proxy forwarder | ✅ Active |
