...
Personalization preservation tests added: tests/fixtures/personalization-original-content.json and tests/unit/personalization-content.test.js retain all four original HTML contracts and 21 source hashes outside the seven mapped generated control-class changes. The first 21 checks pass; four-width dashboard draft captures preserve original rows and contain no page errors, missing assets, unmapped API calls or writes. Full interaction and entry-form review remains open.

Personalization browser review started in tests/e2e/css-unification-personalization.spec.js with synthetic tests/fixtures/personalization-review-data.json. The first six cases cover both dashboard layouts at four widths, original data, keyboard table scrolling, printed headings, filters and failed-load retry. More recovery and form cases remain required before registration.
