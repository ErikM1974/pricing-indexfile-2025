...
| `/dashboards/embroidery-bonus-plan.html` | **NEW 2026-07-25** Rep-facing Q3 2026 embroidery bonus plan — the three ways to earn, why Nika's and Taneisha's goals differ, how the $740K company target was derived, and what counts. Linked from the Mission Control hero ("How this works") and the Flask bonus report's **Bonus Plan** button. Print stylesheet produces a clean PDF handout for a 1:1. A live page rather than a static PDF on purpose: the two `2026_Bonus_Cheat_Sheet_*.pdf` files it replaces drifted out of sync with the code within months. 🔒 `Staff_Page_Access` row restricts it to the two AEs + admin (it shows per-rung payouts). | embroidery-bonus-plan.css, art-hub.css | ✅ Active |
...
| `/shared_components/js/staff-dashboard/controllers/embroidery-bonus-controller.js` | **NEW 2026-07-25** Renders the **Q3 Team Push** strip — company-wide quarter revenue against the shared targets. 🔒 **Shows NO compensation**: every employee opens the staff dashboard, so per-rep bonus dollars live on each rep's own Mission Control instead. Calls `/api/crm-proxy/embroidery-bonus/team`, which forces `scope=team` server-side and returns `reps: {}` — the privacy boundary is in the backend, not this render. **Replaced `garment-tracker-controller.js`** (removed same day: that spiff paid per-garment off a style list and totalled ~$128/~$54 for Q2). No legacy `window.StaffDashboardService` bridge; every number including the targets comes from the API, never a hardcoded constant (the old controller's `BONUS_GOAL = 500`). Loud banner when the backend falls back to built-in targets. | dashboard-events.js, dashboard-errors.js, dashboard-endpoints.js, dashboard-ui-utils.js, `/api/crm-proxy/embroidery-bonus/team` | ✅ Active |
