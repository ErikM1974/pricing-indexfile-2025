...
| `/lib/hashed-pages.js` | **NEW (2026-08-05)** THE list of pages whose assets are content-hashed — imported by BOTH `scripts/build.js` (what to hash) and `server.js` (what to rewrite when serving), so the two sides cannot drift. Tranche 1 = the 4 quote builders; tranche 2 = 26 public storefront pages (index, product, catalog, cart, the custom-* brand pages). Staff pages deliberately excluded for now. | required by scripts/build.js + server.js; locked by tests/unit/build/hashed-pages.test.js | ✅ Active |
