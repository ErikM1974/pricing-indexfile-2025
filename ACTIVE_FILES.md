...
| `/shared_components/js/quote-builder-utils.js` | **NEW** Shared utilities: escapeHtml, formatPrice, showToast, etc. (2026-01-30 consolidation). **`NON_SANMAR_VENDORS` / `vendorLabel()` / `resolveNonSanmarPricingMode()` (2026-08-14)** — vendor label vocabulary (NOT a price; Rule 7 doesn't apply) + the tolerant reader for `Non_SanMar_Products.PricingMethod`, which exists in production as `FIXED`/`FixedPrice`/`Margin`/blank | All quote builders | ✅ Active |
...
| File | Purpose | Dependencies | Status |
|------|---------|--------------|--------|
| `/dashboards/ae-mission-control.html` | AE Mission Control — per-AE cockpit (Taneisha/Nika; admin view-as). **RESTRUCTURED 2026-07-26** (2026-07-19 original): the 16 cards that had stacked on one 3,400px scroll are now a persistent **spine** (Q3 bonus hero with a pace-marker flag, YTD/MTD/streak, 30-day sparkline; a 48px condensed bar takes over on scroll via IntersectionObserver) above **5 lazy-loaded tabs** — Today (The One Thing next-best-action, Needs You Today, order due dates, missing info, SanMar inbound, purchasing), Money (team kicker, bonus & commission, what earned it, where the money is), My Book (Money on the Table), Pipeline (leads/quotes/art/orders + quote-to-order conversion), Wins (records, won back, finished photos). Deep-linkable `#tab=<id>`. Only Today's loaders fire on boot (8 requests, was 9 for everything); other tabs fetch on first open. Role-gated in server.js (`taneisha`/`nika`; admin auto-passes); AEs land here after SAML login. | dash-shell.css, dash-page-helpers.js, **dash-tabs.js**, confetti.js, sanmar-inbound-today.js, **sanmar-inbound.css**, sanmar-invoice-viewer.js, ae-mission-control.css/js | ✅ Active |
...
| `/dashboards/css/ae-mission-control.css` | Mission Control styles (art-hub tokens ONLY — never staff-dashboard/tokens.css, which redefines `--space-5`/`--radius-*`/`--shadow-*`/`--ease-out` with different values and is @layer-wrapped). `aemc-` = the original cards, `mc-` = the 2026-07-26 tabbed shell (spine, sticky tablist + condensed bar, panels, The One Thing, team kicker, records, photo grid). **The file-wide `[hidden]{display:none!important}` guard is load-bearing** — without it the admin view-as pill rendered on both reps' own cockpits, and tab panels would not hide at all. Tabs are modeled on unqualified-leads.css, deliberately NOT art-hub `.tab-button` (white on `--art-theme` is ~2.9:1 and fails AA), and styled off `[aria-selected]` so visual and accessible state cannot diverge. Includes the first `prefers-reduced-motion` block in the dash-page family, plus mobile fixes for iOS zoom-on-focus (16px inputs), 44px touch targets, and three separate sources of sideways page scroll. | dash-shell.css, art-hub.css | ✅ Active |
| `/dashboards/past-due-orders.html` | **NEW (2026-08-10)** Past Due Orders — every ShopWorks order that missed its requested-ship date and is neither shipped nor invoiced, grouped by sales rep, with the blanks status (no PO raised / ordered / partial / received) from the PurchaseOrders mirror. **NOT SanMar-only** — the SanMar Inbound board only sees orders with freight arriving, which is a fraction of the shop; vendors here include Logomark, Augusta, Richardson, Supacolor and "no PO raised yet". Window selector 30/60/90d, print stylesheet for the mono laser. Any logged-in staff (unlisted in `Staff_Page_Access` = any-staff, and that is deliberate — production, receiving and purchasing all have reason to look). Backed by the company-wide branch of the same `buildDueDates()` that feeds Mission Control's per-rep card, so the two can never disagree. | dash-shell.css, dash-page-helpers.js, past-due-orders.css/js, `/api/crm-proxy/ae-dashboard/due-dates-all` | ✅ Active |
| `/dashboards/js/past-due-orders.js` | Past Due Orders controller. ⚠️ Uses a plain **same-origin** `fetch` with `credentials:'same-origin'`, NOT `DashPage.fetchJson` — fetchJson prefixes `APP_CONFIG.API.BASE_URL` (the Heroku proxy), and this endpoint is a `requireStaff` forwarder on THIS server that attaches the CRM secret; routing it through the proxy base would 401. On failure it renders nothing and shows the error banner, because a blank table would read as "nothing is late". | DashPage (showError/hideError), `/api/crm-proxy/ae-dashboard/due-dates-all` | ✅ Active |
| `/dashboards/css/past-due-orders.css` | Past Due Orders page styles (art-hub tokens only). Past-due rows carry weight as well as colour so they survive greyscale; `@media print` forces the late badge to solid black because the shop's mono laser drops backgrounds — same rule as the SanMar inbound sheets. | dash-shell.css, art-hub.css | ✅ Active |
...
| `/staff-dashboard-v3/past-due-badge.js` | **NEW (2026-08-10)** Live past-due count on the sidebar's Past Due Orders link. Three states: red count when something is late (the section icon also swaps from a clock to a red alert triangle via `data-pastdue` in dashboard-v3-theme.css), green "All clear" on a **confirmed** zero, and **no badge at all** when the count can't be fetched — an unverified all-clear is the exact shape of Erik's #1 rule. Polls every 10 min, skips hidden tabs, and re-checks on tab focus. Same-origin fetch (requireStaff forwarder), not the proxy base. | `/api/crm-proxy/ae-dashboard/due-dates-all` | ✅ Active |
...
| `/tests/ui/test-ae-mission-control.html` | Mission Control UI harness — real controller + CSS over stubbed fetch (no SAML/Caspio); exercises the spine, all 5 tabs, view-as, kit/outreach modals, inbound rep-scope. **Mirrors the page's `<main>` verbatim and must be edited in the same commit** — it fails SILENTLY otherwise: the JS finds the old ids in the old layout and renders happily while testing a layout that no longer ships. | ae-mission-control.css/js, **sanmar-inbound.css**, dash-tabs.js, test-ae-mission-control-stub.js | ✅ Active |
...
| `/tests/ui/test-quick-access-layout.html` | **NEW (2026-07-29 IA reorg)** Quick Access layout harness — **fetches the real `staff-dashboard-v3/index.html` at runtime** and lifts its Pride Wall + Quick Access zone in, so it can't drift from production like a copied fixture would. Deliberately omits the hidden Caspio auth DataPage, which otherwise redirects the whole tab to the Caspio login and makes the page uninspectable in a headless browser. Resizable stage exercises the real container queries with no window resize. | the 6 staff-dashboard CSS files, tool-grid-controller.js, test-quick-access-layout.js/.css | ✅ Active |
| `/tests/ui/test-quick-access-layout.js` | Assertions for the layout harness — 6 cards in the intended order, every badge equals its card's unique-href count, Pricing spans 2 tracks at ≥880px container and 1 below, Reference spans the full row, no card overflows its cell, Pride Wall collapse hides the photo track, Frequently Used fits one line. Measures real `getBoundingClientRect` values, and probes collapsed `<details>` with `checkVisibility()` — a skipped subtree keeps reporting its last laid-out rect, so a height===0 test silently passes at no width. | tool-grid-controller.js | ✅ Active |
| `/tests/ui/test-quick-access-layout.css` | Harness-only chrome (sticky width toolbar, pass/fail report, resizable stage). Styles nothing inside the dashboard markup — that must come from the real stylesheets. | — | ✅ Active |
...
| `/dashboards/css/payroll.css` | Payroll dashboard styles (NWCA green, no inline; negative balances flagged). **(2026-08-03)** Grouped Vacation/Sick table header, carryover `raw → slip` cells, flag banners, and a 6-row print slip. | - | ✅ Active |
...
| `/dashboards/quote-management.html` | Quote management dashboard (search, edit, manage quotes) | quote-management.css, **sanmar-inbound.css**, js/quote-management.js, sanmar-inbound-today.js | ✅ Active |
...
| `/shared_components/js/staff-dashboard/controllers/sales-goal-controller.js` | **v3** Header goal chip — progress fill + % + "$X / $3M" (label derived from `ANNUAL_GOAL`, aria-valuenow kept current). Pace/days-left/projected-EOY code deleted 2026-08-27 (their DOM ids left with the big banner 2026-07-20; the branches silently no-oped). | dashboard-ui-utils.js | ✅ Active |
...
| `/shared_components/js/staff-dashboard/controllers/nav-access-controller.js` | **NEW (2026-07-28)** Role-aware sidebar — reveals `[data-requires-role]` blocks (today: the whole Administration section) only when `GET /api/crm-session/me` reports a matching permission. REMOVES the node otherwise, so admin tools also drop out of the Ctrl+K registry, which harvests from the live DOM. Fails CLOSED and logs loudly. **UX layer only — the real gate is `lib/page-access.js` + Staff_Page_Access.** | /api/crm-session/me | ✅ Active |
| `/shared_components/js/staff-dashboard/controllers/tool-grid-controller.js` | **NEW (2026-07-29 IA reorg)** Quick Access housekeeping, no network. (1) Derives every `.tool-category-count` badge from that card's unique `a.tool-btn[href]` count, so the hand-typed numbers can never drift again (Specialty Products had read "5" over six links for two months). Unique-href, not anchor-count, because Quoting lists Quick Quote twice on purpose. (2) Persists open/closed state for any `details[data-collapse-key]` — today the Pride Wall and the Reference card — into the `widgetCollapse` store slot. | dashboard-store.js | ✅ Active |
