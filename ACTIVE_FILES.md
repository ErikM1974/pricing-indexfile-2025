...
| `/scripts/build-handbook-pdf.py` | **NEW (2026-05-27)** Permanent PDF generator — fetches employee-handbook parent + 22 chapters via the public proxy API, assembles into single HTML with cover/TOC/page-breaks, renders to PDF via xhtml2pdf. Re-run any time chapters change. Output → `forms/Employee-Handbook-Latest.pdf`. **Not a temp script** — keep. | Python 3 + xhtml2pdf (`pip install xhtml2pdf`) | ✅ Active |
| `/forms/Employee-Handbook-Latest.pdf` | **NEW (2026-05-27)** Auto-generated 37-page Employee Handbook PDF. Regenerate via `python scripts/build-handbook-pdf.py` after any chapter edit in the Policies Hub. Linked from `/pages/handbook.html`, `/pages/policies-hub.html` hero tile, and the parent policy's Body_HTML. | (generated by build-handbook-pdf.py) | ✅ Active |
