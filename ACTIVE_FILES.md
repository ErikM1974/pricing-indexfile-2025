...
| `/shared_components/js/staff-dashboard/controllers/embroidery-bonus-controller.js` | **NEW 2026-07-25** Renders the Embroidery Bonus card — per-rep activation bounties (new / reactivated embroidery accounts), growth ladder vs each rep's own baseline, shared $3M team kicker, and an expandable "Who to call" dormant-account list. **Replaced `garment-tracker-controller.js`** (removed same day: that spiff paid per-garment off a style list and totalled ~$128/~$54 for Q2). Talks to the API directly — no legacy `window.StaffDashboardService` bridge — and reads every goal/rate from `/api/crm-proxy/embroidery-bonus` config rather than a hardcoded constant (the old controller's `BONUS_GOAL = 500`). Renders a loud banner when the backend falls back to built-in rates. | dashboard-events.js, dashboard-errors.js, dashboard-endpoints.js, dashboard-ui-utils.js, `/api/crm-proxy/embroidery-bonus{,/dormant}` | ✅ Active |
