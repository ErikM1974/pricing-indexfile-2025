...
| `/dashboards/sanmar-payables.html` | **NEW (2026-07-20)** SanMar Payables — native replacement for the Caspio-embedded "Sanmar Vendor Portal" DataPage + manual email-parse workflow. TWO tabs: **Invoices** (live SOAP pull for a date range, filter by PO/invoice#, per-row SanMarInvoiceViewer drill, **Download ShopWorks CSV** = exact 7-col vendor-1002 payables incl. credits; MRKFUND badged+non-exportable) and **Marketing Fund** (SanMar 2026 allotment $35,110.95 → vendor 2425; remaining = allotment − net MRKFUND YTD, year-end projection, month bars, its own 2425 CSV). Admin+accountant via Staff_Page_Access. Phase 2 adds paid/imported match + reconciliation. | dash-shell.css, dash-page-helpers.js, sanmar-payables.css/js, sanmar-invoice-viewer.js, /api/staff/sanmar-invoices/by-date | ✅ Active |
| `/dashboards/js/sanmar-payables.js` | SanMar Payables controller — same-origin `/api/staff/sanmar-invoices/by-date?start=&end=` live pull; Invoices tab (terms-aware: Net30→1002 exportable incl. credits, MRKFUND→disabled, $0→unchecked; ShopWorks CSV M/D/YYYY + INV-/CR- + `$`-amounts via injection-hardened csvCell + UTF-8 BOM) + Marketing Fund tab (quarterly YTD pulls filtered to MRKFUND, editable allotment, remaining/projection math, month bars, 2425 CSV); SanMarInvoiceViewer drill; esc() everywhere, visible errors (no silent empty table) | DashPage, sanmar-invoice-viewer.js, /api/staff/sanmar-invoices/by-date | ✅ Active |
