...
| `/tests/ui/test-admin-nav.html` + `.js` + `.css` | **NEW (2026-07-28)** Browser QA harness for the Administration reorg. Pulls the REAL sidebar out of `staff-dashboard-v3/index.html` via DOMParser (so the Caspio auth embed never fires and can't redirect a static load) and drives it with the real controllers + stylesheets. Role dropdown simulates admin/accountant/sales/staff/anon/endpoint-failure; asserts grouping, 18-links-preserved, aria-expanded, collapse independence, no clipping, and fail-closed removal | sidebar-controller.js, nav-access-controller.js, staff-dashboard CSS | ✅ Active |
