...
| `/dashboards/js/sanmar-inbound-today.js` | "SanMar Inbound Calendar" modal — pick any day → that day's POs (PO+WO, line items w/ color/size, per-box contents, logos), box labels + PDF; month-calendar picker. **Every print/label action re-pulls with `refresh=true` first** (`syncBeforeOutput()`, 2-min window) — a cached payload silently short-shipped the 2026-07-29 report by 4 POs. **Any page that loads this MUST also load `sanmar-inbound.css`** | APP_CONFIG, **sanmar-inbound.css**, /api/sanmar-orders/inbound-today + /daily-inbound (proxy) | ✅ Active |
| `/dashboards/css/sanmar-inbound.css` | **NEW (2026-07-29)** Self-contained styles for the SanMar Inbound modal, its 6 printable report profiles (`#sit-print-sheet`) and the 8.5×11 per-box receiving labels (`#sit-label-sheet`). Split out of quote-management.css, which ae-mission-control.html never loaded — the modal opened there `position:static` ~3,800px below the fold and, worse, without `body.sit-printing > *:not(#sit-print-sheet){display:none}`, so printing a report or label printed the whole dashboard. Carries its own border-box reset and restates `.modal`/`.modal-content`/`.btn-cancel` at `.modal.sit-modal` specificity, so it depends on no other sheet. **Load it wherever sanmar-inbound-today.js loads** | — | ✅ Active |
