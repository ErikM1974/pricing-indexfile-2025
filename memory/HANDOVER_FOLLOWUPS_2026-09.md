...
Standing edit/deploy authorization continues. **Frontend v2026.09.09.1 / Heroku 2080 is LIVE**, full SHA **efc3c7630d64d1b7fa816ff95edde265842c40b0**, actual slug verified 2026-09-09T09:32:20.8215035Z. Exact-source CI **34332748897** passed all four jobs including actual live money/calculator parity on **6b197548bbb43ca9ab5a602a7059edb2c260fe98**. All fifteen brand page sources and fifteen public aliases serve the unified version. Three raw runtime assets and the actual referenced source/hashed build assets match approved bytes. Five regression staff routes remain gated; ten earlier public/reference/form shells return200. No business writes or notifications.

**99 reviewed application pages live;126 pending**, from225 application/served-archive HTML sources. Another79 tracked HTML sources are excluded (12 archive,14 email,50 fixtures/templates,3 retired). All26 training pages are reviewed/live. Generated document/style owners remain separate unfinished work. Ignored training/api-test-runner.html is not tracked/deployed; leave it alone.

### Current batch and next steps

- [x] Fifteen brand guides deployed after the twelve webstore guides (.20). Shared storefront navigation/typography/controls; original text/SEO/images/products/links retained.31 focused browser cases,168 source/ownership/runtime checks,1261 paper content blocks. Raw CSS graph65,000 ->72,788 bytes; shared component coverage increased, so do not claim a byte reduction.
- [x] Five staff references implemented in isolation: commission-structure, embroidery-bonus-plan, seo-strategy, forms-library, data-entry-guide. Branch **codex/staff-reference-family** based on reviewed brand commit **a6d59feca31b1a4419c1f963bb91c063e7d03539**. Working directory **C:/Users/erik/.codex/visualizations/2026/09/07/01a07d90-9a4c-7e70-9e4e-c196377b7c6b/training-final-worktree**. App edits currently UNCOMMITTED and owned by this session; do not deploy them yet. node_modules is a junction; no .env copied.
- [ ] Finish staff print review and focused checks, commit reviewed owned files explicitly (including new shared assets and tests), then integrate and run full local checks/exact-source CI/actual rollout verification. Candidate2026.09.09.2 references pre-versioned. Only after verified rollout:104 reviewed/121 pending.
- [ ] Continue remaining storefront/staff/calculator/builder families and generated document owners. The application-wide cleanup is not complete.

### Verification / resume evidence

Brand release:213 unit suites/5,152 passed/four existing skips;88 DOM,4 accessibility unit,84 quote parity;321 mocked browser in444.6s;16 remaining browser in4.1min,3 optional screenshot skips. All5 live calculator surfaces including EMB/CAP additional-logo tiers. Build/lint/types/296 CSS/HTTP boot pass;485 routes/24 modules unchanged.

Staff review:403 targeted source/ownership tests passed;21 focused browser cases passed before final print warning polish. Four widths1440/768/390/320, keyboard contents/tables, failure/malformed/fallback/retry and populated paper tested with business writes blocked. Original prose and monetary examples retained; dynamic prices/configuration still come from APIs. Verified live access baseline:four dashboards302 for anonymous callers, data-entry guide200; preserve this existing boundary. Paper comparison normalizes CSS-added decorative arrows and text-transform only. Print pages commission9 ->5, SEO15 ->11 without deleting copy. Repeat relevant checks after final changes before committing.

Artifacts: **C:/Users/erik/.codex/visualizations/2026/09/07/01a07d90-9a4c-7e70-9e4e-c196377b7c6b**. brand-guide-{release-record,source-ci,heroku-verification,live-verification,local-gates}.json are authoritative. Staff originals/access baseline, focused config3422, screenshots and rendered paper review saved alongside. Runtime live verification must handle both raw versioned aliases and minified content-hashed page assets; source hash differs from minified bytes and JS hash excludes the appended sourcemap comment. Compare raw assets to Git, hashed assets to the passed local build plus manifest. Do not rerun one-shot installers/finalizers/release helpers. After uncertain pushes inspect actual Heroku SHA before retrying.
