...
- **Verified in preview:** all deps+fns+buttons load; Steve modal opens with form prefilled (company/status/notes/placement) + PNG attached; customer cold-start guard, with-request modal+note, invalid-email guard all correct; zero console errors. **⚠ NOT yet tested: the LIVE round-trip (real POST /api/artrequests create + real customer EmailJS send) — avoided to not create junk prod records/emails. PENDING: one guarded live test submission (Erik deletes the test ArtRequest after).**
