...
### Caspio `q.distinct` is silently IGNORED — and unordered `q.groupBy` pagination DROPS ~5-10% of groups per run (2026-07-12)
**Problem:** `/api/all-styles` (feeds teamnwca sitemap-products.xml) reported 569 styles when the live catalog has 3,197 — PC54 itself (#1 seller, 20K units printed) was absent from the sitemap Google received.
**Root Cause:** Two stacked: (a) Caspio v3 tables/records accepts `q.distinct` but IGNORES it (returns raw rows) → the "distinct" scan crawled the 251k-row bulk table and hit fetchAllCaspioPages' 20-page safety cap at 20k rows = "styles that appear in the first 8% of the table". (b) The `q.groupBy` rewrite WITHOUT `q.orderBy` returned a DIFFERENT style set every run (2,824 / 2,649 / 2,916) — grouped output has no stable order, so `q.pageNumber` pages skip/duplicate groups randomly, and the two runs' sets diverged in BOTH directions.
**Solution:** `q.groupBy` + `q.orderBy: 'STYLE'` (proxy products.js `/all-styles`): two identical runs → identical 3,197 live styles; Discontinued-only styles filtered out. Proxy commits 7a693a5 + 4518f20.
**Prevention:** (a) NEVER use `q.distinct` — use `q.groupBy`. (b) EVERY multi-page Caspio query needs `q.orderBy` on a stable column, or pages silently skip rows (audit other fetchAllCaspioPages callers). (c) A suspiciously small "distinct" count vs the table's known scale, or any count that changes between identical runs, is the tell — run twice and diff.
...
**Root Cause:** Without `q.orderBy`, Caspio v3 has no stable row order across `q.pageNumber` requests — page N of run 2 ≠ page N of run 1, so the union skips/duplicates rows. (Related: `q.distinct` is silently IGNORED on tables/records — use `q.groupBy`; groupBy OUTPUT rows paginate too.) Drift shows while the table is being written (daily SanMar sync); quiet-hours runs can match by luck, so a clean spot-check proves nothing.
