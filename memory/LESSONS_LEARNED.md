...
**Problem (recurring):** Mockup images break in art request gallery + customer approval view. Box returns 404 for fileIds still referenced by Caspio. Steve previously had to manually re-link 21 designs. Nika reported again on #52850 after #52844/#52846. Thumbnails show generic IMG badge; download alerts "This mockup file no longer exists in Box".
**Root Cause:** Upload flow is correct (stores proxy URL `/api/box/thumbnail/{fileId}` from Box's authoritative response). Files get deleted **after** upload via Box web UI cleanup — `BOX_ART_FOLDER_ID` ("Steve Art Box 2020") is a shared-access folder; anyone can delete files still referenced by active ArtRequests. Secondary path: `DELETE /api/box/file/:fileId` endpoint had no reference guard — the Ruth digitizing Box panel "X" button could wipe files used by active mockups.
**Solution (defense in depth, 2026-04-22):** (1) Frontend 404 detection — `handleImageError()` in art-request-detail.js / mockup-detail.js / art-actions-shared.js HEADs the proxy URL on image error; on 404 flips the slot into "File missing — Re-upload" card (customer view → soft "Mockup is being updated — contact your rep" with tel link). Staff Re-upload reuses the empty-slot popover. (2) Backend upload hardening — `verifyBoxFileAccessible()` HEADs fileId after upload; returns 502 `BOX_VALIDATION_FAILED` if not fetchable. Wired into all 3 upload endpoints. (3) DELETE endpoint guard — `findBoxFileReferences()` scans ArtRequests (5 URL fields) + Digitizing_Mockups (7 fields) for `thumbnail/{fileId}` before deleting. Returns 409 `FILE_IN_USE` with ref list unless `?force=true`. Fails closed on Caspio error. (4) `/api/box/download/:fileId` 404 now includes `code: 'BOX_FILE_NOT_FOUND'`. (5) Fixed `sharedUrl` → `proxyUrl` at box-upload.js:1383 (Digitizing_Mockup_Versions.File_URL was `undefined`).
**Prevention:** Caspio-referenced Box files CAN still be deleted out-of-band — we can't technically block that. Defense in depth: detect broken files early (HEAD on img error), surface clearly ("File missing — Re-upload" not "IMG" badge), make repair one-click (inline Re-upload), prevent new holes (post-upload verify + DELETE reference guard). Long-term: Box retention policy on `BOX_ART_FOLDER_ID` so deletes go to trash.
