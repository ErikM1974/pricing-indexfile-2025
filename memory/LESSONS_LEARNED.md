...
## Integration: Caspio Triggered Actions Don't Fire on REST API Updates (2026-03-06)

**Problem:** Art Hub revision workflow needed to send email notifications to sales reps when Steve sends back a design for revisions or marks it complete. Initially assumed Caspio's built-in triggered actions (email on record update) would handle this.

**Root Cause:** Caspio triggered actions (email notifications, workflows) ONLY fire on DataPage form submissions — they do NOT fire when records are updated via the REST API. Since the Art Hub uses the caspio-pricing-proxy backend to update `ArtRequests` records via REST API, no Caspio-side automation would trigger.

**Solution:** Used EmailJS (client-side email service already in use for quote builders) to send notifications directly from the browser. Two templates: `template_art_revision` (orange header, revision notes box) and `template_art_completed` (green header, art time/billing box). Fire-and-forget pattern — email failure doesn't block the workflow.

**Prevention:** When building features that update Caspio via REST API, always plan for client-side or server-side notifications. Don't rely on Caspio triggered actions for REST-updated records.

**Tags:** `[Pricing Index]` `[caspio-proxy]`
