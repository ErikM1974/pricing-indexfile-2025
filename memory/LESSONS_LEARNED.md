...
### SanMar ships per CARTON; ShopWorks receives per PO (2026-07-27, ARCHIVED 2026-07-27): **when two systems disagree on the unit of work, every per-PO flag is a lie about some carton** — check the grain before trusting a rollup. `boxDetail` returns EVERY box a PO ever shipped, so scope to the arrival band. And a reconciliation whose only inputs are "what the vendor says changed" can never notice a record the vendor has gone silent about. Full entry in archive.

### Four images failed 643 times each and the log still said "0 still pending" — a failure masking itself three ways (2026-07-27)
- **Problem**: after changing the bandit Thumbnail Box Sync cadence I ran it once to verify, and the log ended `DONE: 8 uploaded, 0 skipped, 4 errored this run; 0 still pending`. The same 4 files had been erroring since 2026-07-17 — **643-644 attempts each, 2,575 of the log's 2,584 HTTP 500s** — and nobody had noticed.
- **Root Cause**: three independent masks. (a) `upload-with-stub` caps uploads at 20 MB (`multer limits.fileSize`, `thumbnails.js:18`) and the files are 21-28 MB, but multer's `LIMIT_FILE_SIZE` is **not caught**, so a precise, permanent condition surfaced as a generic **"An unexpected error occurred"** 500. (b) The agent's summary counted an errored file as *processed*, so `still pending` printed **0** next to 4 errors — the line read healthy. (c) The 2,575 permanent-failure lines **buried ~22 genuinely transient failures** (one attempt each, all succeeded on retry), so the log was useless as a signal either way.
- **Solution**: the agent now checks size client-side against a `MaxUploadBytes` param pinned to the server's multer limit, names each skipped file with its size, and reports `skipped(too large)` as a **separate category** from `errored`. The misleading counter was renamed to `not attempted this run`, which is what it actually measures. Erik's call: 4 files of ~27,600 stay without a thumbnail rather than raise a cap that holds whole files in dyno RAM. Verified: 13 uploaded, 4 skipped(too large), **0 errored**.
- **Prevention**: **a permanent failure must never look like a transient one** — give it its own category, or it will drown the failures you could actually fix. Costing nothing is not the same as harmless: these were 0 Caspio calls (multer rejects before the handler) yet still did real damage as noise. And **never let a counter call an error "done"** — if a summary line can print `0 pending` while items are failing, the number is measuring the wrong thing. When the *same N items* fail every single run, that is a permanent condition, not a retry queue.
...
### A stub-based UI harness cannot see a missing route (2026-07-26, ARCHIVED 2026-07-27): **a mock that stands in for the network can never test the network** — a card 404'd in production for a week while the harness stayed green. Any page whose harness stubs `fetch` needs a separate live probe per route (**401 = registered = pass, 404 = not registered**), cross-checked against a deliberately fake route to prove the probe discriminates. Full entry in archive.

### `requestAnimationFrame` is throttled to ZERO in a background tab (2026-07-26, ARCHIVED 2026-07-27): **never let an animation be the sole writer of a real value** — paint the truth synchronously, then decorate (`paint(to); if (reduceMotion || document.hidden || !to) return;`). A rep's bonus sat at **$0 indefinitely**. `prefers-reduced-motion` is a CSS-only signal — a rAF counter or canvas confetti needs an explicit `matchMedia` check in JS. Full entry in archive.

### Per-item escaping in a list builder: three of five branches escaped, two didn't (2026-07-26, ARCHIVED 2026-07-27): when N producers feed ONE `innerHTML` sink, **escape at the sink, never per producer** — "every caller remembers to escape" is not a property a codebase can hold. Keep an XSS probe fixture (a literal `<b>` in a name) in every list harness; it is what caught this. Full entry in archive.

### One shared `OverlapMinutes` across syncs with DIFFERENT cadences is a silent-data-loss trap (2026-07-26, ARCHIVED 2026-07-27): **overlap must always exceed (that task's cadence + run duration + clock skew)**, so one value shared by scripts running 15 min → 12 h cannot be correct for all of them. Order/PO now read their own `OrdersOverlapMinutes` (20); the shared key stays **30** for thumbnail-metadata's 30-min cadence. Detail → [SHOPWORKS_ODBC_INTEGRATION.md](./SHOPWORKS_ODBC_INTEGRATION.md). Full entry in archive.

### Four hand-maintained brand lists drifted until a top-seller search returned ZERO products (2026-07-25, ARCHIVED 2026-07-27): **same fact in 2+ files ⇒ derive it or test-compare it**, never a "should match" convention — `brands-registry.js` is now generated from the 46 catalog spellings, so it cannot name a brand the catalog lacks. `includes()` keyword matchers need **≥3 chars + longest-first**. Full entry in archive.

### A truthy `[]` pinned an EMPTY brand list for 24h server-side + 6h in every browser (2026-07-25, ARCHIVED 2026-07-27): **never cache an empty collection** — `[]` is truthy, so `/api/all-brands` served its own empty for 24h and was unflushable. Cache only when `length > 0`; an empty result ⇒ **503 `no-store`**, never a cacheable empty 200. Full entry in archive.
...
### A test harness that FORKS the page it tests will drift, and green means nothing (2026-07-27, ARCHIVED 2026-07-27): **any time a test fixture duplicates production markup or data, add a test that COMPARES them** — `scripts/sync-test-harness.js --check` + `tests/unit/harness-drift.test.js`, wired into `npm run test:guards`. Same rule as the four hand-typed brand lists and the pricing matrix: one fact in two places is a bug waiting for a deadline. Full entry in archive.
...
### Author CSS `display` always beats the UA `[hidden]` rule (2026-07-19, ARCHIVED 2026-07-27): any element JS toggles via `.hidden` that ALSO has an author `display` rule **silently never hides** (author origin wins the cascade regardless of specificity) — start every new page/dashboard stylesheet with a file-wide `[hidden]{display:none!important}` guard. Also: when JS builds DOM the CSS contract IS the class string (grep `innerHTML`/`className` on both sides before a refactor); never regenerate DOM nodes during hover/mid-click. Full entry in archive.
...
### One rep, FOUR name shapes across tables — and one is a trap (2026-07-19, ARCHIVED 2026-07-27): per-rep filters hit different shapes (full name / email / free-text `ArtRequests.Sales_Rep` that CHANGED shape mid-history) — proxy `ae-dashboard.js AE_REGISTRY` is the one email→{fullName,firstName} map (sync w/ FE `leads-common.js EMAIL_TO_REP`); verify a rep column's live shape with one query before filtering, never trust map comments; a rep-name map entry matching nothing in `Sales_Reps_2026` is a bug, not a variant. Full entry in archive.
...
### "The style exists" is not "the style is buyable" (2026-07-27, ARCHIVED 2026-07-27): the blog autopilot published a post recommending a **DISCONTINUED** vest because its gate checked existence, not orderability. **When a check gates content, assert the property the content actually claims** — "order this" needs *orderable* (`PRODUCT_STATUS` / the `DISCONTINUED` title prefix are the live signals; catalog rows never disappear). Any write-ahead content bank must RE-verify its facts at **publish** time, never trust authoring time. Full entry in archive.
