...
> - ✅ **Bulk onboarding — locator + staging (2026-07-01):** `caspio-pricing-proxy/scripts/portal-onboard-locator.js` (READ-ONLY by default; `--stage` = dry-run plan; `--stage --apply` = write). Queries Caspio DIRECTLY (proxy client-cred token → **no ~2000-row public-endpoint cap**) for the UNION of customers with a COMPLETED proof from EITHER artist: Steve `ArtRequests` Status∈{Completed,Approved} + real proof (`Final_Approved_Mockup||Box_File_Mockup||BoxFileLink`, **NOT** MAIN_IMAGE_URL) OR Ruth `Digitizing_Mockups` Status=Completed + `Box_Mockup_1|2|3`. Login email resolved = most-recent-ordering active `CompanyContactsMerge2026` contact (`Last_Order_Date DESC`). Writes rows identical to the admin console POST (`Email` lowercased, `id_Customer` digits, `Company_Name`, `Enabled='Yes'`, `Role=''`). Emits `portal-onboard-cohort.csv` (**NOT committed** — holds customer emails). **Counts (2026-07-01):** union **222** (206 Steve / 30 Ruth / 14 both); **143** have a 2026-dated proof (= non-empty My Logos TODAY, portal date-gates display); 213 email-resolved. **STAGED 135** `Customer_Portal_Access` rows (ready-now = has-email + has-2026-content, minus house acct **3739** + already-staged, **deduped by email** — Email is the unique key but **Caspio does NOT enforce it**, so the script must dedup itself). **NO invites sent — provisioning ≠ inviting; reps send the magic link when ready.** Idempotent (skips existing email/id; re-run created 0). Held back: **79** no-2026-content (onboard when the Phase-1 brand-logo table lands), **9** no-email (human picks the contact). Total portal-staged now 138.
> - ⏳ **DEFERRED (Erik wants to discuss) — auto-onboard FUTURE completions:** display already auto-flows (`getPortalData` reads mockups+art live per request — **reference-not-copy, nothing to build**; all future art is 2026+ so it clears the display gate). For auto-ADDING not-yet-provisioned customers: recommend a **NIGHTLY reconcile reusing the locator** that PROVISIONS the confident-email customers and QUEUES the ambiguous/emailless ones into a "Ready to onboard" tab in the admin console — **auto-provision, NEVER auto-invite** (a wrong email only hurts at invite time). Prefer nightly over per-completion triggers (Caspio triggers **don't fire on our REST writes**; nightly keeps ONE home for the email-resolver + denylist + 2026 gate). Ties to task #4.
...
## ✅ "Your Products" → catalog view (one card per STYLE + ordered-color swatches) — SHIPPED 2026-07-01 (FE v2026.07.01.6)
`buildMyProducts` (server.js) now groups the per-(style,color) list by STYLE after enrichment: most-recent color = the card's primary image; every ordered color → a swatch `{name, swatch=COLOR_SQUARE_IMAGE, image}` (resolved via cached `portalStyleRows`+`portalMatchColor`). Returns `products:[{...primary, colors[], colorCount, timesOrdered(sum)}]`. FE `productCardHtml` renders a `.cp-swatch` row + "N colors ordered" subtitle when `colors.length>1` (single-color styles unchanged). Fixes the repeated-garment problem (Takehara 1438: 45 style+color rows → 24 style cards). Re-order modal still opens on the primary color; `buildRecommendations` ownedStyles/houseColor still derive fine from the grouped list. Verified 1438: swatches render, 0 console errors.
