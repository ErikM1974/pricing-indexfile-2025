...
> - ⏳ **Phase 2 (next — the security win):** swap `resolvePortalCustomer` (server.js) to read `req.customerSession.portalCustomer.idCustomer` (not the URL); `/portal/:customerId`→`/portal`+shim; rename `/api/portal/:customerId/*`→`/api/portal/*`+`requireCustomer`; drop `&cid=` from `customer-portal.js` + the two detail JS files. Closes the URL-token IDOR.
