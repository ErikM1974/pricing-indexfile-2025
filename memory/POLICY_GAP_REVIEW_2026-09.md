...
- 🔴 **`/api/files/upload` sits behind the proxy `writeLimiter` (~60 writes per 15 min per IP)**: five parallel upload agents produced ~50 HTTP 429s. One paced uploader at ≥ 16 s between uploads never hit it. Reads (`GET /api/files/<key>`) are not limited.
