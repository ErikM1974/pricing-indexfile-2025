...
- ⏭️ **Open for Erik — beanies/knit caps:** Quick Price (and the product page) price them as caps; a line sheet prices them by the chosen method (Embroidery → flat, matching the EMB builder; Cap embroidery → cap, with a row note). Not changed without a decision. Visors are routed as caps (the builder prices blank-category visors as garments).
...
- Source mappings: stage `classic-restore-2026-09-16` in `tests/fixtures/quick-quote-workflow-source-mappings.json`. Rows were generated per diff hunk and replay-verified to HEAD; regenerate the stage after any further edit to the four hashed files.
