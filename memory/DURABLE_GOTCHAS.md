...
- 🔑 **Caspio paging: ONE model, and page size must be 5..1000.** `fetchAllCaspioPages` used to pull page 1 with `q.limit` then continue with `q.pageSize`+`q.pageNumber=2` — models that disagree on which rows are the first N, so a >1-page scan returned **326 duplicates and silently dropped 326 real rows** (measured on `SanMar_Shipments`). FIXED 2026-08-26 (`1b87492`+`c4534d3`), jest-locked by `caspio-pagination-model.test.js` which asserts the outgoing PARAMS — the bug was invisible in the returned array on any small table. 🔴 The traps if you touch it again: **`q.pageSize` is fussier than `q.limit`** — `q.limit=5000` is OK (Caspio silently caps at 1000, which is WHY 6 routes that asked for 5000 were truncating at 1000 rows until the same fix routed them through real paging) but `q.pageSize=1001` is REJECTED, and `q.pageSize<5` is too, so the 51 `q.limit:1` existence checks must stay on `q.limit` (they used to cost **4 Caspio calls to read one row**: page 1, a rejected page 2, a token refresh, a rejected retry). ⚠️ `q.sortBy` is NOT a Caspio param — it is silently ignored; the correct one is **`q.orderBy`**, and paging without it is undefined.
