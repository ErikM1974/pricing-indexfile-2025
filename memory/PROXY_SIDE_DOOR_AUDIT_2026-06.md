...
- ⏳ **NEXT Phase-1 (non-Inksoft):** `thumbnails` writes/deletes (mass-delete design files; script callers — check headers), `admin/products` (method-scoped inside mixed `products.js`), `shipstation` outbound create/DELETE, proxy `orders` (verify dead first). All method/path-scoped — gate writes only, never the public reads (Rule 9).
