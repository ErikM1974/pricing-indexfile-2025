...
- 🟢 **PHASE 1 COMPLETE (non-Inksoft).** All server-only/CLI/cron-callable holes gated; `SESSION_SECRET` fail-closed guard now LIVE on the FE (shipped with the shipstation deploy; app boots clean). ⚠️ The FE deploy cleared MemoryStore → staff logged out (the #8 problem) — argues for #8 next.
