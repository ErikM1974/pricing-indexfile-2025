...
**Large lightbox image (v2026.07.01.11):** the proxy `/api/box/thumbnail/:id` already supports `?size=large` (box-upload.js:944 → streams the full original for image files, 1024 Representation for PSD/AI/PDF). renderMyLogos appends `?size=large` for the lightbox only (grid stays 256px). Verified 10181: full-res 2541x3294. NOTE: the proof is the internal DTG/production mockup SHEET (shows rep name + "Customer Approved"/"Files Prepared" checkboxes + color-accuracy disclaimer) — real + crisp, but a cleaner customer-only crop is a possible future refinement.
