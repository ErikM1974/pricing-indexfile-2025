...
## ✅ My Logos IMAGE + CLICK fixes (SHIPPED 2026-07-01, FE v2026.07.01.10) — 3 gotchas
Reported: portal showed "just the shirt" + clicking a design errored. Root causes + fixes:
1. **`ArtRequests.MAIN_IMAGE_URL_1` is a plain SanMar GARMENT catalog photo, NOT the design proof.** The real proof lives in `Final_Approved_Mockup` (best) → `Box_File_Mockup` → `BoxFileLink` (all `/api/box/thumbnail/<id>` proxy URLs, ~256px). `projectPortalArt` now exposes those (customer-safe; still NO `Art_Minutes`/`Amount_Art_Billed`/`Artwork_Locations`/file-paths); `getPortalData` art filter accepts any of them; `renderMyLogos` prefers proof over garment. (Digitizing_Mockups `Box_Mockup_1` was already the real mockup.)
2. **`/art-request/:id` and `/mockup/:id` are INTERNAL STAFF pages** (footer literally says "Internal Staff Page") — they 404/error for customers. My Logos cards are now `<div class="cp-logo-card">` that open a **customer lightbox** (`#cp-logo-lightbox`, full design image + title/meta), NOT a link to those pages. FE image-proxy (`server.js:7421`) has NO host allowlist (any https), so Box-thumbnail proxy URLs pass through fine.
3. **Status regex gotcha:** `/approv|complet/` MATCHES "Awaiting **Approv**al" → pending designs wrongly showed the ✓ Approved badge. Fixed to EXACT match: `t==='approved'||t==='completed'`. (Real Status values: Completed/Approved/Awaiting Approval/In Progress/Submitted/Revision Requested.)
Verified 10181 Binford: 8 designs, proof images (box/thumbnail not sanmar), lightbox opens the "Happy Place" mockup, #4026 (Awaiting Approval) NO badge / 7 Completed badged, 0 console errors. ⏳ image is ~256px thumbnail (soft in the 720px lightbox) — a full-size Box endpoint is a possible follow-up.

**Large lightbox image (v2026.07.01.11):** the proxy `/api/box/thumbnail/:id` already supports `?size=large` (box-upload.js:944 → streams the full original for image files, 1024 Representation for PSD/AI/PDF). renderMyLogos appends `?size=large` for the lightbox only (grid stays 256px). Verified 10181: full-res 2541x3294. The proof shown is the DTG/production mockup SHEET (rep name + "Customer Approved"/"Files Prepared" checkboxes + color-accuracy disclaimer). **DECISION (Erik, 2026-07-01): LEAVE AS-IS — that sheet is exactly what Steve already sends the customer, so it is familiar, NOT an internal leak. Do NOT clean up / crop it.**

**SanMar-fallback REMOVED (v2026.07.01.12):** MAIN_IMAGE_URL_1 (a SanMar catalog stock photo of the blank garment/model, e.g. `PC78_jetblack_model_front.jpg`) was the last-resort fallback → showed "just the model" for designs with no Box proof. Erik: only want the design. renderMyLogos now uses ONLY `Final_Approved_Mockup||Box_File_Mockup||Box_File_Link`; a design with no proof on file is SKIPPED (not shown). Verified 10181: 8→7 cards, #40154 (proof-less, only File_Upload_One=/Artwork/Binford.png + a SanMar url) dropped, 0 SanMar-stock cards. (File_Upload_One is a raw Caspio Files path, not a displayable proof URL.)
