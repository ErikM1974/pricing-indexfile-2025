...
1. **Caspio write** — A `quote_sessions` row is created with prefix `GOLF` (e.g. `GOLF0504-1`). Customer name/email/phone, company, player count, and tournament-date/interests/notes (packed into the Notes field) are saved. 30-day expiry. Status: `Open`. Implemented via the standard [base-quote-service.js](../shared_components/js/base-quote-service.js) class — same pattern as DTG, EMB, RICH, etc. quote builders.
   - **Auto-built starter line items (2026-06-10):** the submit handler now also writes `quote_items` so the customer's `/quote/:id` page is populated (was empty — showed "Total items: N" from `TotalQuantity` but "No items in this quote"). `buildLineItems()` in [golf-tournament-showcase.js](../shared_components/js/golf-tournament-showcase.js) picks ONE representative style per checked interest (preferred SKU then config group; `INTEREST_GROUPS`/`INTEREST_PREFERRED`), uses the `?prefill=` style as a dedicated line for its category, and prices each live at the player-count tier via `EmbroideryPricingService`. Qty per line = player count; `subtotal`/`TotalQuantity` reflect the built items. `hats` interest has no catalog group → no auto line (stays in Notes). Player count `other` → lead-only (no items). Pricing failure → graceful lead-only save. **Sales team still refines styles/colors/sizes** — these are starter estimates (footer already says "Final pricing confirmed in your quote").
