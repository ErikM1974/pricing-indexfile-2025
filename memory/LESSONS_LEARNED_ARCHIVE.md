...
## Leads Board/List toggle looked dead — author CSS `display` always beats the UA `[hidden]` rule (2026-07-19) (archived 2026-07-27)
...
## SanMar ships per CARTON; ShopWorks receives per PO — the mismatch hid inbound boxes two different ways (2026-07-27, found by reconciling a SanMar freight manifest against our own report) (archived 2026-07-27)
- **Problem**: Erik compared SanMar's 7/24 freight manifest (DEC-006920, 12 POs / 152 pcs) against the Daily Inbound Report. Ten POs matched line for line. Two did not: **PO 113682** reported **70 pcs / 3 boxes** when only **6 pcs in one carton** were inbound *and* was simultaneously suppressed from the report entirely; **PO 113781** (Superheat LLC, WO 142568) appeared on **no arrival day at all** — a box would have landed with nobody expecting it.
- **Root Cause**: three unrelated causes that all present as "the report is wrong". (a) `boxDetail` came from SanMar's live shipment feed, which returns **every box the PO ever shipped** — a split shipment therefore reported its whole history, and its whole cost, on the first carton's arrival day. (b) ShopWorks records `date_Received` **per PO** but SanMar ships **per carton**, so a PO counted in on the 21st collapsed to "✓ Received" and dropped off the totals, Mikalah's checklist and the box labels while a carton that shipped on the 24th was still in transit. (c) The catch-up sweep discovered work only by **asking SanMar what changed** (`allOpen` + wide `lastUpdate` + invoice discovery) — so a PO SanMar stops reporting is invisible to all three: once it completed it left `allOpen`, its `Last_Sync_Date` froze, and being a no-invoice marketing-fund order it never surfaced in invoice discovery either. It sat on `confirmed` with **zero shipment rows** for three consecutive syncs.
- **Solution**: `scopeBoxesToArrival()` keeps only cartons whose Caspio shipment row passed the arrival band (falling back to ALL boxes when tracking can't be matched); `isFollowOnShipment()` suppresses the received collapse only when the arriving carton predates the count-in, and a new amber **"↩ Follow-on shipment"** badge carries onto the card, the print sheets, the receiving checklist and the box label; **Discovery C** asks *our own table* for non-terminal orders the daily sync has stopped touching (`Last_Sync_Date` 2+ days stale), which is the only discovery path that survives SanMar going quiet. Fresh A/B candidates take the ingest budget first and truncation now logs the deferred POs by name. 11 tests pinned to the real 113682 carton data.
- **Also added — the actual completeness guarantee**: Discovery C treats the symptom; the disease was that **every** discovery path was **PO-first** ("what shipped for the POs I know about"), and a PO-first system can confirm what it holds but can never prove it holds everything. SanMar's OSN **`queryType=3`** inverts it — *"everything you shipped since X"*, which is precisely the freight-manifest/PSST view — and the call was **already in the codebase**, buried in the manual 90-day `/backfill`. Promoted a rolling 3-day version to **Phase 0 of the daily catch-up**; POs it finds with no order row feed into the same ingest pipeline. One SOAP call + one paged Caspio read per day.
- **Prevention**: **when two systems disagree on the unit of work — carton vs PO — every per-PO flag is a lie about some carton.** Check the grain before trusting a rollup. And **a reconciliation whose only inputs are "what the vendor says changed" cannot notice a record the vendor has gone silent about** — it needs at least one path that asks the *inverse* question (give me everything in this window) or one rooted in your own data. Before building either, **grep for the inverse query first: it may already exist in a batch/backfill path** and only need promoting to the daily one. The general shape here — an in-flight record frozen mid-lifecycle because the thing that advances it stopped mentioning it — is worth hunting anywhere we mirror an external system. Erik's Rule #4 sets the tie-break throughout: over-reporting a box is recoverable at the dock, silently hiding one is not, so both new filters fail OPEN and a failed sweep throws rather than returning "nothing shipped".

## A stub-based UI harness cannot see a missing route — a card 404'd in production for a week while the harness stayed green (2026-07-26) (archived 2026-07-27)
...
## `requestAnimationFrame` is throttled to ZERO in a background tab — an animated counter left the bonus reading $0 (2026-07-26) (archived 2026-07-27)
...
## Per-item escaping in a list builder: three of five branches escaped, two didn't (2026-07-26) (archived 2026-07-27)
...
## One shared `OverlapMinutes` across syncs with DIFFERENT cadences is a silent-data-loss trap (2026-07-26) (archived 2026-07-27)
...
## Four hand-maintained brand lists drifted until a top-seller search returned ZERO products (2026-07-25) (archived 2026-07-27)
...
## A truthy `[]` pinned an EMPTY brand list for 24h server-side + 6h in every browser — homepage Brands menu went blank (2026-07-25) (archived 2026-07-27)
...
## A test harness that FORKS the page it tests will drift, and green means nothing (2026-07-27) (archived 2026-07-27)
- **Problem**: `tests/ui/test-ae-mission-control.html` is a hand-copied fork of `dashboards/ae-mission-control.html`. Across one day it silently kept the OLD bonus hero, the OLD tab bar and the OLD Today panel while the real page was rebuilt three times. Every run passed — against markup that no longer shipped. It was re-synced by hand four times, and each re-sync was noticed only because a browser check rendered nothing.
- **Root Cause**: the fork is unavoidable (the page is SAML-gated, so the harness can't fetch it at run time) but nothing enforced the copy. "Remember to update the harness" is not a mechanism. The failure is invisible by construction: both files parse, both render, the tests go green, and the only symptom is that the harness exercises a page nobody will see.
- **Solution**: `scripts/sync-test-harness.js` copies the shared regions (tablist, hero, all `.mc-panel` sections, drawers) and `--check`s them; `tests/unit/harness-drift.test.js` fails the build on any byte difference and is wired into `npm run test:guards`. On its first run it immediately caught two drifts nobody had spotted. Crucially it also derives coverage FROM the page — every `.mc-panel` must be in the guarded set, so a newly-added tab can't be silently unguarded, which is exactly how the Call List panel drifted.
- **Prevention**: **any time a test fixture duplicates production markup or data, add a test that compares them.** Same rule as the four hand-typed brand lists (2026-07-25) and the pricing matrix: one fact in two places is a bug waiting for a deadline. If the duplication can't be removed, make the divergence fail loudly — and derive the list of what to check from the source of truth, never a second hand-maintained list. Also: a "green" suite over stale fixtures is worse than no suite, because it buys false confidence.

## "The style exists" is not "the style is buyable" — the blog autopilot published a post recommending a DISCONTINUED vest (2026-07-27) (archived 2026-07-27)
...
## Two files named `artwork-upload.js` collided on one global — and the deploy script bumped 3 quote builders (2026-07-25) (archived 2026-07-26)
- **Problem**: I created `pages/js/artwork-upload.js` for the public sticker/banner pages. `shared_components/js/artwork-upload.js` already existed — a 458-line rep-facing multi-file widget loaded by **3 quote builders**. Both exported `window.ArtworkUpload`. Caught only because the deploy cache-buster bumped `?v=` on three Rule-8 builders for a file they don't use.
- **Root Cause**: two independent failures that happened to fire together. (1) Same global name — nothing broke *today* because no page loads both, but any page that ever did would get whichever `<script>` came last, presenting as "upload stopped working," never as a name clash. (2) **The deploy cache-buster matches on the LAST TWO PATH SEGMENTS**, so `js/artwork-upload.js` matched both files; every future deploy touching either would re-bump three builders forever.
- **Solution**: renamed the public one to `pages/js/instant-quote-artwork.js` exporting `InstantQuoteArtwork` (matching the `instant-quote.css` naming already on those pages) and reverted the three builder bumps. Kept as **separate modules deliberately** — merging a customer artwork step into a widget 3 builders depend on would be a Rule 8 change bought for nothing but a shared name.
- **Prevention**: **basename uniqueness is load-bearing in this repo, not just style** — `grep -r "<basename>" --include=*.js -l` before creating any shared-ish JS file, and check the global it exports. A duplicate basename in a *different* directory is exactly as dangerous as one in the same directory, because the tooling keys on the tail of the path. Corollary: when a deploy script bumps a file you didn't touch, stop and read why — that's the tooling reporting a real collision, not noise to click through.

## A sizeless product vanished from its own quote while still counting toward the total (2026-07-24) (archived 2026-07-26)
...
## A "money on the table" list that was 5x wrong, and the deprecated helper that made it un-runnable (2026-07-25) (archived 2026-07-26)
- **Problem**: the Q3 bonus roadmap's "almost there" list told Nika she had 26 accounts and $1,350 of nearly-earned bounties. The real number was 4 accounts / $250. Separately, the endpoint that built it timed out at 10 minutes; the computation actually takes **2.2 seconds**.
- **Root Cause**: (a) the list included every account under the $1,000 bounty threshold, but an account that ordered embroidery *inside* the dormancy window is a **repeat** customer and earns $0 no matter what it orders — 23 of the 26 were repeats. (b) The timeout was `makeCaspioRequest`, which is `@deprecated` and does `JSON.stringify(response.data)` **as a log argument on every response** — megabytes of serialization per 1,000-row page, and unsilenceable by stubbing `console.log` because the argument is evaluated first. It also already unwraps `.Result`, so a `resp.Result` keyset loop reads `undefined` and silently returns zero rows. Third, unrelated: multi-term `<>` filters in a Caspio WHERE were **4.5x slower** than a plain date range (16.8s vs 3.7s) while returning fewer rows.
- **Solution**: list C now only includes accounts that would actually classify as New or Reactivated. `pullOrders` uses `fetchAllCaspioPages` exclusively; type exclusions moved from the WHERE into JS.
- **Prevention**: **an "opportunity" list must apply the same qualification rules as the payout, or it lies** — model the payout on each row before showing it. Never use `makeCaspioRequest` for bulk reads (it's deprecated *and* logs whole bodies); never push multi-term `<>` filters into a Caspio WHERE on a large table. And when diagnosing a hang, **write raw output to a file — piping through `grep`/`tail` buffers and hides the progress you need** (cost two wasted 10-minute cycles here).

## A retired order type made 5 accounts look brand-new and would have overpaid the bonus (2026-07-25) (archived 2026-07-26)
- **Problem**: building the Q3 2026 Embroidery Bonus, "accounts that have never embroidered with us" counted 8 new accounts for Nika in 2026 Q1. The real number is 3. Separately, her Q3 2025 embroidery read $199,421 when it was actually $236,841 — which would have set her bonus baseline ~$37K too low, i.e. a target she'd clear without doing anything.
- **Root Cause**: **cap embroidery was its own `id_OrderType` (1 "Caps") until it folded into 21 "Custom Embroidery" at the 2025 Q3→Q4 boundary** ($44,341 in 2025 Q3 → $1,095 in Q4 → $0 from 2026 Q1). Any query scoped to type 21 alone therefore (a) understates all pre-2026 embroidery and (b) can't see a customer whose only history was caps, so it reports them as never having embroidered. Two more traps in the same data: `id_OrderType=16` "Wow Embroidery" is 78 orders at **$0.00** (internal redos — counting it inflates account counts with free work), and ORDER_ODBC embroidery goes back to **2000**, not 2019, so a 2019-bounded lookback also invented "new" accounts (Kingfisher Charters embroidered in 2018).
- **Solution**: two separate scopes in `embroidery-bonus.js` — `EMB_ORDER_TYPES = [21]` for current-quarter revenue, `EMB_HISTORY_TYPES = [21, 1]` for every lookback, with no lower date bound; 16/31/22/42 excluded. Both are Caspio-config-overridable (`Order_Type_Ids` / `History_Order_Type_Ids`). A regression test asserts Nika's 2026 Q1 new count is 3.
- **Prevention**: **before trusting any multi-year query on a categorical ID, census that ID by year** — a type whose volume drops to exactly $0 was retired into another one, and a type that appears mid-history was split out of one. `ORDER TYPE × YEAR` took one query and caught all of this. Never assume a historical scope equals the current scope just because the label is the same, and never bound a "first ever" lookback to a convenient recent year. Detail → [EMB_BONUS_Q3_2026.md](./EMB_BONUS_Q3_2026.md).

## An eligibility flag that was right 11 times out of 19, and a test that passed because it tested nothing (2026-07-26) (archived 2026-07-26)
- **Problem**: webstore accounts had to stop earning the embroidery bonus (they already pay through the Online Store commission, so counting them double-paid). The obvious signal, `Sales_Reps_2026.Inksoft_Store`, flags only **11 of the 19 Hops N Drops locations** — Bonney Lake and Lacey read `false` while ordering through the store every month. A filter trusting the flag would have looked correct, passed review, and still paid on 8 excluded accounts.
- **Root Cause**: the flag is maintained upstream in ShopWorks and re-synced every 15 minutes, so it can't be corrected here and its gaps are invisible from this side. Also, the leak was never the order type — InkSoft orders are type 31 and were never counted; it was a webstore **customer** placing a direct type-21 order. Filtering by order type can't see a customer-level rule.
- **Solution**: detect with a **union** — `Inksoft_Store = true` **OR** any type-31 order history — which catches 19 of 19. One predicate feeds the ladder revenue, the bounties, the dormant list and all three target lists, and it **fails safe**: a missing `Exclude_Online_Store` config column still excludes, because over-paying silently is the worse failure on a hand-edited table.
- **Prevention**: (a) when a boolean is maintained by another system, **spot-check it against behaviour before trusting it** — one query comparing the flag against actual order history exposed the gap in seconds; (b) **assert on the real-world names**, not the flag — a test asserting `Inksoft_Store === false` passes while the bonus is still wrong; (c) **a test that passes trivially is not a test.** The "rate is continuous / no dead zone" assertion held only because both reps sit below the 85% start, where 0 points pays $0 whatever the arithmetic does — it would have passed against a completely broken rate. It now also runs against a closed quarter where a rep finished at 125.7%, so it exercises a real $2,439. Check *why* a new assertion passes the first time. (d) Excluding revenue means the **baseline must move in the same deploy** — Nika's book was ~45% webstore, so filtering her revenue while leaving the old $235,000 goal would have made her target unreachable. Detail → [EMB_BONUS_Q3_2026.md](./EMB_BONUS_Q3_2026.md).

## products/search silently hid every PRODUCT_STATUS='New' style — misread as "not in the DB" (2026-07-23) (archived 2026-07-26)
...
## Expert audit of EMB/SCP/DTF builders — three DEFECT CLASSES, 55 findings fixed (2026-07-07) (archived 2026-07-19)
Full inventory: QUOTE_BUILDER_EXPERT_AUDIT_2026-07-07.md.
...
## quote-view.js carried a DEAD hardcoded extended-size upcharge map `{2XL:2,3XL:4,4XL:6,5XL:8,6XL:10}` (Rule-9 latent violation, 2026-07-06) (archived 2026-07-19)
...
### Box shared-static logo links 301 instead of serving the image (2026-07-17, archived 2026-07-19)
...
## Archived 2026-07-21 (via /memory-maintain — oldest resolved one-time fixes; 1-line keep-alive stubs left in the active file)
...
### Leads CRM hardening batch — 4 reusable UI/data patterns (2026-07-19, multi-agent audit)
Fixed 20 verified bugs (see git `fix(leads):` commits). The patterns worth keeping:
- **A rebuilt `<select>` must write its effective value back to state.** `setOptions()` reset the DOM `<select>` to '' when the prior selection vanished but left `state.status` stale → the board filtered by an invisible value showing "0 of N" the user couldn't clear. Any dropdown rebuilt from data must return/sync the value it actually holds.
- **Never announce a write before it confirms.** The drawer "Link customer" logged an immutable "Linked" timeline row + flipped the button + autoloaded orders synchronously, before the PUT resolved — a failed save left a false record, and the autoload read the field before it was set. Gate all success-UI (activity log, button flip, re-render) on the resolved promise (`saveField` now returns true/false).
- **A create-retry on ANY error duplicates the row.** The proxy POST retried with a fresh id whenever the first POST threw — a post-write timeout (Caspio committed, response timed out) then created a DUPLICATE lead. Read the row back before retrying; only retry when it truly isn't there.
- **A 15s client timeout ≠ the server didn't act.** Outreach send reported "Email NOT sent" on an AbortError even though the server delivered → duplicate emails on retry. Detect the timeout and tell the user it MAY have gone out; verify before resending.
Also: identity/audit fields (`Updated_By`/`Created_By`) must be stamped from the verified session server-side, never trusted from the client body; `element.value =` (not innerHTML) for every prefill of attacker-controlled lead data; CSV export of attacker data needs `=+-@` formula-injection hardening.
...
### `git add -u` in a SHARED checkout deployed another session's half-done work → prod 503 outage (2026-07-19, v2026.07.19.15)
...
## Condensed keep-alive stubs retired from LESSONS_LEARNED.md (2026-07-26)
...
## One rep, FOUR name shapes across tables — and one is a trap (2026-07-19, AE Mission Control build) (archived 2026-07-27)
