...
**✅ PHASE 2 SHIPPED LIVE — `v2026.06.09.1` (2026-06-09).** Billable shipping + UPS estimator, folded into the WA tax base at all 4 sites (screen/PDF/submit/save) via the single `effectiveShipFee()` rule + a DTF/SCP-parity SHIP line item so /quote+/invoice show the shipping row + foot. Adversarial review (6 lenses) ZERO money bugs; full Preview matrix + ShopWorks TEST push **OF-0057 conversion-verified** (cur_Shipping 25 + shipping-taxable Notes survive MO→OnSite); 1037/1037 unit. **DTG now at full EMB/DTF/SCP parity — Phases 0+1+2 all shipped. Do NOT redo.** Loose ends: Erik void test orders OF-0055/0056 (P1) + OF-0057 (P2). ⚠ PRE-EXISTING (NOT P2, separate task `task_9e507a1a`): Caspio `Quote_Sessions.LTMFeeTotal` rejects decimals → LTM DTG quotes (qty<24) 400 on SAVE (the share-link path; the push path is fine). Detail: "PHASE 2 COMPLETE" section below.
