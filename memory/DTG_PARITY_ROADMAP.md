...
**Loose ends for deploy:** (1) **Real ShopWorks TEST push** of a non-pickup DTG quote with a fee → confirm Notes On Order shows "Shipping (taxable)" + Taxable Base + cur_Shipping carries the fee (Erik voids the test order, like Phase 1's OF-0055/0056). (2) Then merge `dtg-phase2-wip`→`develop` + `/deploy`. (3) **PRE-EXISTING bug flagged (NOT Phase 2, separate task):** Caspio `Quote_Sessions.LTMFeeTotal` column rejects decimals → saving any LTM DTG quote (qty<24) 400s; Phase 1's verified save was a no-LTM quote so it was never hit. Fix = change the column type to allow decimals (verify EMB/SCP/DTF aren't also affected).
