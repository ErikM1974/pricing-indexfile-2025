# ⭐ NEXT SESSION — START HERE (DTG parity — all 3 phases SHIPPED)

**✅ PHASE 2 SHIPPED LIVE — `v2026.06.09.1` (2026-06-09).** Billable shipping + UPS estimator, folded into the WA tax base at all 4 sites (screen/PDF/submit/save) via the single `effectiveShipFee()` rule + a DTF/SCP-parity SHIP line item so /quote+/invoice show the shipping row + foot. Adversarial review (6 lenses) ZERO money bugs; full Preview matrix + ShopWorks TEST push **OF-0057 conversion-verified** (cur_Shipping 25 + shipping-taxable Notes survive MO→OnSite); 1037/1037 unit. **DTG now at full EMB/DTF/SCP parity — Phases 0+1+2 all shipped. Do NOT redo.** Loose ends: Erik void test orders OF-0055/0056 (P1) + OF-0057 (P2). ✅ FIXED 2026-06-09 (was a separate pre-existing bug): Caspio `Quote_Sessions.LTMFeeTotal` is INTEGER → LTM DTG quotes (qty<24, amortized 49.92) used to 400 on SAVE; now `Math.round()`-ed at both write sites (handleSaveQuote + server.js OF route), verified live (DTG-2026-049 saves). Detail: "PHASE 2 COMPLETE" section below + LESSONS_LEARNED.

**✅ PHASE 1 SHIPPED + LIVE — `v2026.06.08.23` (2026-06-08).** The DTG flagship has full tax UI + wholesale + save-fidelity at EMB/DTF/SCP parity. Verified live + a real ShopWorks wholesale push (OF-0055 → GL 2203). **Do NOT redo Phase 0 or Phase 1.** Full detail: the "PHASE 1 COMPLETE" section lower in this doc.
...
- ✅ PHASE 2 (estimator + billable shipping) — **SHIPPED LIVE `v2026.06.09.1`** (2026-06-09), TEST push OF-0057 conversion-verified. See "PHASE 2 COMPLETE" section below.

## PHASE 2 COMPLETE — BUILT + VERIFIED, PENDING TEST PUSH + DEPLOY (2026-06-09, branch `dtg-phase2-wip`)
**What shipped (8 commits):** `effectiveShipFee()` — the SINGLE pickup-zeroing rule (pickup→0, read by ALL consumers; mirrors recomputeTaxRate as the single tax authority); `#dtgShipFee` field + UPS-Ground estimator wired via Phase 0's `configure()` `ship.fee` slot + `estimateHooks` (collectProducts mirrors priced rows, onApplied syncs DOM→state); the fee folded into the **tax BASE + total at all 4 sites** — renderSummary (Shipping row), computePriceQuoteFromState, dtgPrintQuote (preTaxSubtotal incl fee + shippingFee), submitToShopWorks (+body.ship.fee +body.breakdown.shipping); resetForm re-seeds `fee:0` (reset-bleed guard); loadSavedDtgQuoteForEdit restores fee from Notes.shipping.fee + re-syncs `#dtgShipFee`. **dtg-quote-page.js:** mirror DTF/SCP — `TotalAmount` products-only PRE-tax, write a `SHIP` line item (LineTotal=fee), `TaxAmount` on (subtotal+fee). **server.js:** OF route `cur_Shipping = ship.fee` (was hardcoded 0); `buildOrderNote` taxableBase=subtotal+shipping + a Shipping line across all branches (React-OF backward-safe — sends no breakdown.shipping → 0). CSS + cache-bust `v2026.06.09.1`.
**Adversarial review (workflow w6b8lz7n1, 6 lenses + synthesis): ZERO money bugs** — all 4 sites foot identically + pickup-safe + server.js correct. Caught ONE correctness gap (FIXED): baking the fee into TotalAmount with no SHIP item footed but HID the shipping line + inflated the Subtotal on /quote+/invoice (readers detect shipping ONLY via the SHIP item) → switched to the DTF/SCP SHIP-item convention. Minors: pre-existing 1¢ PDF-vs-screen rounding (shared generator, out of scope); server.js comment fixed.
**Verified LIVE (Preview 3010, real priced PC54 rows):** taxed-fee (non-pickup, manual 10%, fee $25) screen==save==PDF==$254.01, fee IS taxed (23.09 on 230.92 base, Shipping row shows); pickup with a STALE $25 fee → effectiveShipFee zeroes it everywhere (no ship row, grand $226.72); UPS estimator (Seattle 98101 → $16.55, 5lb/1box/zone2) + live DOR rate (10.55%) → fee taxed at destination; **real save DTG-2026-048** (qty 48, fee $25 @ 10%) → saved TotalAmount=600 products-only + SHIP item=25 + TaxAmount=62.5 → **/quote AND /invoice both render the Shipping row + foot to $687.50** → edit-reload restored fee $25 → test quote DELETED (clean). **Jest:** `dtg-tax-base.test.js` +4 fee cases (incl. stale-pickup guard); 1037/1037 unit pass.
**TEST push CONVERSION-VERIFIED (2026-06-09): OF-0057** (PC54 M×24 + $25 ship, Seattle 10.55%) — Erik supplied BOTH the ManageOrders payload AND the OnSite conversion JSON. ✅ `cur_Shipping: 25` SURVIVES the MO→OnSite conversion (was hardcoded 0 pre-P2). ✅ Notes On Order echo all 10 lines incl. the 2 NEW Phase 2 lines "Shipping (taxable): $25.00" + "Taxable Base: $349.00 (… WAC 458-20-110)"; Tax Amount $36.82 is on the $349 shipping-INCLUSIVE base; Total with Tax $385.82 foots. ✅ TaxTotal 0 (manual). Note (pre-existing, NOT P2): Tax Account note shows "2200.105 — 10.50%" for a 10.55% rate — findTaxAccountByRate nearest-bucket; exact rate+amount are in the Notes, tax applied manually. **Erik: void OF-0057 (+ still OF-0055/0056 from Phase 1).**
**Loose ends for deploy:** (1) ✅ DONE — TEST push verified (OF-0057 above). (2) Merge `dtg-phase2-wip`→`develop` + `/deploy`. (3) **PRE-EXISTING bug flagged (NOT Phase 2, separate task):** Caspio `Quote_Sessions.LTMFeeTotal` column rejects decimals → saving any LTM DTG quote (qty<24) 400s; Phase 1's verified save was a no-LTM quote so it was never hit. Fix = change the column type to allow decimals (verify EMB/SCP/DTF aren't also affected).
