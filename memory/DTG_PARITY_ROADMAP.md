# ⭐ NEXT SESSION — START HERE (DTG Phase 2 → TEST push + deploy)

**✅ PHASE 2 BUILT + VERIFIED LIVE — branch `dtg-phase2-wip`, PENDING TEST PUSH + DEPLOY (2026-06-09).** Billable shipping + UPS estimator, folded into the WA tax base at all 4 sites (screen/PDF/submit/save) via the single `effectiveShipFee()` rule, with a DTF/SCP-parity SHIP line item so /quote+/invoice show the shipping row + foot. Adversarial review (6 lenses) found ZERO money bugs; full Preview matrix verified (real save DTG-2026-048 cleaned up). **NEXT: do a real ShopWorks TEST push of a non-pickup DTG quote with a fee (Erik voids it), then merge `dtg-phase2-wip`→`develop` + `/deploy`.** Full detail + loose ends: the "PHASE 2 COMPLETE" section lower in this doc. **Do NOT redo Phases 0/1/2.**
...
- ✅ PHASE 2 (estimator + billable shipping) — BUILT + VERIFIED LIVE on branch `dtg-phase2-wip`, **PENDING TEST PUSH + DEPLOY** (2026-06-09). See "PHASE 2 COMPLETE" section below.
