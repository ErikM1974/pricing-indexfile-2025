...
> ## ✅ SHIPPED UPDATE — Caspio-driven RBAC (2026-06-30)
> The #2 "move role-of-record into Caspio" NEXT is **DONE**, but **NOT** via the directory Role field. **Key finding:** the Caspio directory's built-in per-user `Role` field is **auth-typed** (constrained to defined app-access roles) — a free-text PUT returns 400 `"value doesn't match the data type of field 'Role'"`. And **Caspio Groups are NOT exposed in the REST API** (no `/groups` endpoint; the user object has no group field). So neither the directory Role nor Groups can drive app RBAC.
> - **Solution = a normal Caspio TABLE `Staff_App_Roles` (Email, Role)** — created + populated via API (`caspio-pricing-proxy/scripts/create-staff-app-roles-table.js`). Roles: `admin` (erik), `accountant` (bradley — financial tier), `sales` (taneisha/nika), `art` (Steve/ruth), `shipping` (mikalah), `production` (brian), `staff` (jim = normal user). Erik edits this **table** (Caspio → Tables, NOT Directories) → effect on next login, no deploy.
> - **App reads it at login:** proxy `GET /api/staff-app-role?email=` (`src/routes/staff-app-roles.js`, requireCrmApiSecret-gated) → FE `fetchStaffRole()` (`server.js`) in the SAML ACS handler → `staffSaml.permissionsFromRole(role,email)` (`lib/staff-saml.js`) → session. **Replaced the hardcoded `STAFF_PERMISSIONS` map.** Fail-safe: lookup error → no elevated perms. `admin` = full (incl. house/policies-admin/per-rep); per-rep dashboards still keyed by identity so taneisha/nika keep theirs.
> - **GOTCHAs:** Caspio create-table wants UPPERCASE type keywords (`STRING`=Text255, `TEXT`=Text64000, NOT "Text (255)") + auto-creates the PK (don't declare AUTONUMBER). Directory writes + the constrained Role field are blocked for Claude by the access-control guardrail → Erik runs `--apply`. Live: proxy v855+ / FE `922348bb`. Dead code: `server.js` `CRM_PERMISSIONS` (line ~487) now unused.
>
> ## ✅ SHIPPED — table-driven PAGE access (2026-06-30, FE `74741ad9`)
> Second table **`Staff_Page_Access`** (`Page` unique, `Allowed_Roles` csv, `Allowed_Emails` csv, `Description`) = which `/dashboards/*.html` pages are restricted to which roles/people. Holds only EXCEPTIONS; unlisted page → any logged-in staff. Created via `caspio-pricing-proxy/scripts/create-staff-page-access-table.js` (seeded art-invoices→admin,accountant + commission→admin).
> - **Engine:** proxy `GET /api/staff-page-access` (`src/routes/staff-page-access.js`, gated) → FE `getPageAccessRules()` (60s cache, **stale-on-error** so a restricted page never silently opens) → the `/dashboards` middleware (`server.js` ~2447) calls `userMayAccessPage(crmUser, rule)`: **admin override** (admin sees all, can't self-lock) → unlisted=any-staff → else role-in-Allowed_Roles (matched vs derived perms) OR email-in-Allowed_Emails. The 3 per-rep dashboards (taneisha/nika/house) keep their earlier `requireCrmRole` gates (run first).
> - **Management model = two Caspio tables Erik edits (no deploy):** `Staff_App_Roles` (who=which role) + `Staff_Page_Access` (which page=which access). **RULE:** when building a NEW staff dashboard page that should be RESTRICTED, add a `Staff_Page_Access` row (unlisted defaults to any-staff). ✅ The Erik-only "Access Admin" dashboard UI is now BUILT (`dashboards/access-admin.html`, hard-gated `requireCrmRole(['admin'])` at `server.js`).
> - **⚠️ Named-individual / Erik-ONLY pages (2026-07-09):** the **admin override** (`userMayAccessPage`) means a `Staff_Page_Access` row alone CANNOT exclude other admins — a row with `Allowed_Emails=erik@…` is really "Erik **+ any admin**," and is only truly Erik-only while Erik is the sole admin. For a page that must stay private even if someone else later gets the `admin` role, register an **explicit route BEFORE the `/dashboards` static mount** using the **`requireCrmEmail([emails])`** helper (`server.js`, defined next to `requireCrmRole`; email-keyed, **bypasses the admin override**) — same precedent as `access-admin.html`'s role gate. First use: `/dashboards/caspio-api-reference.html` (Erik-only Caspio REST API v4 reference).
...
- ✅ **THE FLIP — DONE & verified (2026-06-29, release v1477).** (a) `staff-login.html` button → `/auth/saml/login`; (b) `requireStaff` middleware gates `/staff-dashboard.html` + the `/staff-dashboard-v3` routes + all `/dashboards/*.html` (login page + non-HTML assets excluded → no loop; redirects logged-out users to SSO, never hard-locks); (c) `/api/crm-session` (server.js:2127) NO LONGER mints from a posted name — echoes the verified SAML session or 401s. **Hole CLOSED — verified on prod:** forgeable `POST /api/crm-session {name:Erik}` → 401 (was 200/admin); `/staff-dashboard.html` + `/dashboards/ae-dashboard.html` logged-out → 302 to SSO; `staff-login.html` → 200; SAML login + homepage healthy. Rollback: `heroku releases:rollback`.
