...
- ⏳ **NEXT:** (3) **Real round-trip test** — a staff member visits `https://www.teamnwca.com/auth/saml/login`, logs in via Caspio, lands on dashboard; confirm `https://www.teamnwca.com/api/crm-session/me` shows their verified email + permissions (`via:'saml'`). (4) **FLIP** (the hole-closer): replace forgeable `/api/crm-session` (server.js:2124) + add `requireStaff` to ungated dashboards (server.js:2330/2392) + point `dashboards/staff-login.html` at `/auth/saml/login`. (5) Later: durable session store (Phase 4, share w/ #6) + proxy side-door (Phase 5, requireCrmApiSecret).
