...
> - **Management model = two Caspio tables Erik edits (no deploy):** `Staff_App_Roles` (who=which role) + `Staff_Page_Access` (which page=which access). **RULE:** when building a NEW staff dashboard page that should be RESTRICTED, add a `Staff_Page_Access` row (unlisted defaults to any-staff). **NOT YET BUILT:** an Erik-only "Access Admin" dashboard UI (2 panels: Users→roles, Pages→access) editing both tables via gated endpoints — the engine is ready for it.
