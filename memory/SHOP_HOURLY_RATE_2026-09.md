...
**Calculator RETIRED (2026-09-03, Erik: "keep it simple"):** `/calculators/shop-services/` and the `SHP` prefix are DELETED (app config, quote-formatter, Quote Mgmt branch, dashboard link, CLAUDE.md). Reasoning: menu math is price × qty with a $100 floor, walk-in jobs go straight into ShopWorks (that order is the record), and no real quote ever went through it. The supplies math survives as a rep-only line in House Rule 3 (cost ÷ 0.53: $1→$1.89, $2→$3.77, $5→$9.43, $10→$18.87). The "Screen Print & Other" course is gone from the Menu too (those fees live in the screen-print builder) so the REP copy also prints on ONE page. Caspio SHOP rows untouched — the Menu is their only reader now.
