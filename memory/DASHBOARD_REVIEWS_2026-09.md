...
- **CSP report-only stream** (Heroku logs): the only violations are `script-src-elem: inline` on `vendor-portals/sanmar-invoices.html` — the Caspio DataPage embed injects inline scripts. ⏭️ **Enforce-day backlog**: Caspio embeds need a nonce or `'unsafe-inline'` scoped to those pages before the policy can be enforced; nothing in our own code trips it.
  - **Styles (2026-09-17):** `style-src` keeps `'unsafe-inline'`, and the quote builders still need it for runtime markup. The four builder pages have no static `style=""` attributes: EMB/SCP/DTF hidden markup uses `hidden data-qb-hidden`, which `quote-builder-utils.js` moves to `style.display` before the builders run. `css-unification-quote-builder-workflows.spec.js` serves EMB/SCP/DTF with `style-src-attr 'none'`, and the alternate-workflows spec does the same for DTG. About 95 runtime `style=""` writes remain in the EMB/SCP/DTF builder JS (product rows, SPR modal, ShopWorks import, push status, `getSwatchStyle`, …). About 7 more build email/print HTML, which is fine. 🔑 Font Awesome's unlayered `.fas{display:var(--fa-display,inline-block)}` beats the layered `[hidden]`, so hidden icons set `--fa-display:none`. Chrome's UA sheet gives `select:disabled` `opacity:.7`.
