...
- **CSP report-only stream** (Heroku logs): the only violations are `script-src-elem: inline` on `vendor-portals/sanmar-invoices.html` — the Caspio DataPage embed injects inline scripts. ⏭️ **Enforce-day backlog**: Caspio embeds need a nonce or `'unsafe-inline'` scoped to those pages before the policy can be enforced; nothing in our own code trips it.
  - **Styles (2026-09-17):** `style-src` keeps `'unsafe-inline'`, and the quote builders still need it. Served with `style-src-attr 'none'`, the EMB, SCP and DTF builders break: their HTML has 85/28/17 static `style=""` attributes, and the hidden `display:none` ones show up, including the share-quote modal over the page. About 98 runtime `style=""` writes remain in builder JS (EMB product-rows/spr-modal/shopworks-import/save-push, `getSwatchStyle`, quote-share-modal and quote-session `display:none`, …). About 7 more build email/print HTML, which is fine. DTG is clean (0 static; the shared runtime pieces became classes on 2026-09-17), and `css-unification-quote-builder-alternate-workflows.spec.js` serves it with `style-src-attr 'none'`.
