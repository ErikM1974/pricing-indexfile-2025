...
- **CSP report-only stream** (Heroku logs): the only violations are `script-src-elem: inline` on `vendor-portals/sanmar-invoices.html` — the Caspio DataPage embed injects inline scripts. ⏭️ **Enforce-day backlog**: Caspio embeds need a nonce or `'unsafe-inline'` scoped to those pages before the policy can be enforced; nothing in our own code trips it.
  - **Styles (2026-09-17):** `style-src` still keeps `'unsafe-inline'` (other pages need it), but the four quote builders no longer do: no static or runtime `style=""` in their HTML or builder JS. EMB/SCP/DTF hidden markup uses `hidden data-qb-hidden`, which `quote-builder-utils.js` moves to `style.display` before the builders run; runtime templates use classes; swatches use `getSwatchAttrs()` + `qbPaintSwatches()` (CSSOM). `css-unification-quote-builder-workflows.spec.js` serves EMB/SCP/DTF with `style-src-attr 'none'` at load and while their runtime templates render (rows, import preview, pricing review, push status, combobox, size popup, DTF config error); the alternate-workflows spec does the same for DTG. Left alone on purpose: email/print HTML (`embroidery-quote-service.js`, `quote-formatter.js`), and the pricing warning banner and DTF config error, which style themselves through CSSOM because other pages load that JS without the sheet. 🔑 A report-only `style-src-attr 'none'` console message carries the script URL and line, which finds each writer; an empty `style=""` is also a violation. 🔑 Font Awesome's unlayered `.fas{display:var(--fa-display,inline-block)}` beats the layered `[hidden]`, so hidden icons set `--fa-display:none`. Chrome's UA sheet gives `select:disabled` `opacity:.7`.
...
# ERIK'S ANSWERS BATCH (2026-09-07, `v2026.09.07.1`) — alert email, goal row, rates, Rule 9, import + sample locks

- **Money-path alerts now EMAIL** (`server.js` `staffAlert` → `staffAlertEmail`): `heroku config` proved NEITHER Slack webhook was ever set, so every `alert3DT` / `alertQuotePay` since they were written reached only Papertrail. Both now go through one pipe: console.error + Slack (if a hook appears) + EmailJS **`template_staff_alert`** (built in Erik's EmailJS account through his Chrome, service `service_1c4k67j`, params `to_email / subject / message / source`) to `ALERT_EMAIL_TO` (default erik@). Verified end to end: a Heroku one-off dyno sent through the template → EmailJS 200, row in Email History. 🔑 `api.emailjs.com` is unreachable from Erik's LAN (TLS interception) — test sends must run on Heroku (`heroku run:detached … node -e`, read the result in `heroku logs`; `heroku run` attached times out on this network, and git-bash's `heroku run` breaks on the `C:\Program Files` path — use PowerShell).
- **`CO-ANNUAL-GOAL` created** (Service_Codes PK 307, UNIVERSAL / Rules / SellPrice 3,000,000) via `POST /api/service-codes` with the `X-CRM-API-Secret` from `.env` — the goal chip's ⚠ fallback is gone. Also corrected two stale SHOP Notes (`SHOP-BENCH-QH` said $90/h, `SHOP-JOB-MIN` said $75). Erik confirmed the posted rates: **bench $100/h, machine $150/h** — the rows already charged exactly that (25 / 37.50 per quarter hour). ⏳ Still open: `VOL-HOUR-RATE` 30.09 is a COST per PAID hour multiplied by BILLABLE minutes (SHOP_HOURLY_RATE § 19-22); Erik's answer was about SELL rates, so the cost row was left as is — needs one more decision (30.09 vs 67).
- **Rule 9 reworded** (Erik: builders doing their own math IS the design): the engine replicates the four `*-pricing-service.js`, held identical by `web-quote-cart-parity` (which DOES cover DTF — the old "no DTF parity test" note was stale).
- **ShopWorks import — no bug found, the missing lock written**: `emb-import-apply-services.test.js` runs the real `_applyImportedFullBack` / `_applyImportedCapLogo` / `applyServiceResults` with state shared and the row factory spied: primary Full Back + FB line = charged once; Left Chest + FB = both; 25K floor; cap logos land on `globalAL.cap`; service-only orders keep their charge as DECG-FB / AL-CAP / AL rows. The three exported functions were unexported before. Real-order verification (a full-back import) is still the only proof left.
- **Sample request → Leads — no bug found, and NO real request has ever gone through**: `/api/crm-proxy/form-submissions?formIds=sample-request` returns `[]` on live (2026-09-07), and EmailJS history shows no `template_sample_request` send in the visible window. `sample-request-lead.test.js` locks the vocabulary sites and `createSampleLead` (House → blank rep, company fallback, stash dedupe, never throws). First real free-sample request: confirm an `SRQ…` card on `/dashboards/leads.html`.
- **Voiding OnSite orders is desktop-only** — ManageOrders + push APIs are create/read only; the ODBC account is read-only. Erik voids `CAP0825-4781` / `CAP0825-7724` in OnSite himself.
