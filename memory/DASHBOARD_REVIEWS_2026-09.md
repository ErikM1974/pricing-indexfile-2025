...
4. Checked the other calculators: DTF builds its tier buttons from the API and reads `LTM_Fee` per tier (see DTF tier check below); embroidery/cap pages show the documented 1-7/8-23/24-47/48-71/72+ headers (stable, LTM $50 on 1-7 from the API); `screenprint-manual-pricing.js` has the same typed strip but is loaded only by an ARCHIVED page.
