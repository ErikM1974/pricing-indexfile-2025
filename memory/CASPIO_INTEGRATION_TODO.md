...
- ⏳ **STAGE C remaining:** the `/mockup/:id?view=customer` & `/art-request/:designId?view=customer` DETAIL pages still read raw public endpoints (`mockup-detail.js`/`art-request-detail.js` customer-view branch) — add `GET /api/portal/:customerId/mockup/:id` + `/art-request/:designId` (authorize row.customer==cid) and repoint those branches. Until then, single-item detail still exposes sensitive fields.
