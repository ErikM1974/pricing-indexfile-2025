...
- [SANMAR_FTP_INTEGRATION.md](SANMAR_FTP_INTEGRATION.md) — how the `Sanmar_Bulk_251816_Feb2024` product master (garment CASE_PRICE for all quote builders) is refreshed from SanMar FTP `SanMarPI-Bulk-*.csv`: admin page `/dashboards/sanmar-ftp-integration.html` (download buttons via server.js `/api/staff/sanmar-ftp/*`, needs `SANMAR_FTP_PASSWORD`) + local `sanmar-ftp-box/Download-SanMar-Pricing.bat`; Caspio upsert on UNIQUE_KEY (2026-07-22)
- [SANMAR_SHOPWORKS_PARTS.md](SANMAR_SHOPWORKS_PARTS.md) — admin converter `/dashboards/sanmar-shopworks-converter.html` (client-side): SanMar price list → ShopWorks Parts import CSV. Auto-detects ShopWorks integration list (fixup: sizes-in-descriptions + `_2XL`→`_2X`, `_XXL` kept) vs raw feed (full conversion). Verified vs existing parts; `sts_LimitSize` base `----11` / 2XL+XXL `1111-1` / else `11111-` (2026-07-23)
...
- [EMB_BONUS_Q3_2026.md](EMB_BONUS_Q3_2026.md) — Q3 2026 Embroidery Bonus (2026-07-25), replaced the garment spiff: activation bounties ($75 new / $50 reactivated embroidery accounts) + growth ladder vs each rep's own baseline (Nika $235K / Taneisha $100K) + $3M team kicker ($700K/$740K company Q3). 🔴 **Read the data traps before ANY historical embroidery query**: type **1 "Caps" folded into 21 at 2025 Q3→Q4** (union both or you understate history and invent "new" accounts), type 16 "Wow Embroidery" is $0.00 internal redos, ORDER_ODBC embroidery reaches back to **2000** not 2019, and attribution is `Sales_Reps_2026` never the order row. Also: the two books have **opposite Q3 seasonality** (Nika 1.38× H1, Taneisha 0.96×), and a **378-account / $1.64M dormant embroidery runway** drives the "who to call" list. ⏳ ERIK: create `Rep_Bonus_Config` + `EmbroideryBonusArchive` (spec in the doc) — until then the endpoint uses built-in defaults with a loud warning
...
- [STICKER_MULE_COMPETITIVE_2026-07.md](STICKER_MULE_COMPETITIVE_2026-07.md) — SM price teardown (our sheet = SM × ~1.76, but our savings % ladder ties/beats theirs) + `/custom-stickers` & `/custom-banners` build decisions; **🔒 the 2-field quote gate stays by design**
