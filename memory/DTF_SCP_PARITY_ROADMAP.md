...
- **NEXT (the "polished like the flagship" finish — NOT a correctness blocker per the audit): the shipping ESTIMATOR** (Commits 4-6 / old Phase 1): extract `estimateShipping`/`perBoxForCategory`/box-density + `openShippingModal` into `quote-order-summary.js` (parameterize `_cfg.estimateHooks={collectProducts,onApplied,btn,result}`, keep `_cfg.estimate=()=>QuoteOrderSummary.estimateShipping()` so the band's Re-estimate auto-lights); EMB rewire zero-behavior-change (lift box-math BYTE-FOR-BYTE — validated vs real UPS invoices, do NOT refactor); DTF adopt (`collectProducts: dtfQuoteBuilder.products.map(p=>({style:p.styleNumber,sizeBreakdown:p.quantities}))`); SCP adopt (`collectProductsFromTable` filtered `!isService`). +`#ship-residential` toggle. Punch list: `C:\Users\erik\AppData\Local\Temp\punchlist.md` (regenerate from audit `wo2slacjf` if gone). Then Phase 5 polish + final re-audit.
