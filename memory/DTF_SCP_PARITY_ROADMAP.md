...
- **✅ ORDER-SUMMARY BAND PARITY COMPLETE across EMB + DTF + SCP.** Remaining roadmap: **Phase 1** (shared shipping ESTIMATOR + modal for DTF+SCP — move `estimateShipping`/box-density/`openShippingModal` into the module, per-builder `collectProducts()`), then **Phase 4** (tax-exempt/wholesale + residential parity for DTF/SCP), **Phase 5** (polish + visual sweep + routing-doc update).
