...
- **✨ POLISH PASS COMPLETE (2026-06-08, v2026.06.08.16–.19) — all 5 items done + verified live.** (1) **Visual sweep** (audit w2g7bwyss): extracted the estimator chrome to shared `.estimate-ship-*` CSS — fixed the ALL-CAPS residential label (`.customer-field label` uppercase → `text-transform:none`), the ballooned mobile checkbox (`input{padding:10px}` → `padding:0` on checkbox/radio), the 28px button tap-target (→44px @≤768px); `.st-actions` flex-wrap; `#wholesale-row` 12px+green. (2) **EMB button** unified indigo→green (`.estimate-ship-btn`, all 3 builders match). (3) **Stale comments** + **SCP wholesale toggle-OFF** now re-derives the rate (exempt 0 / out-of-state 0 / WA DOR re-fetch, parity w/ DTF). (4) **EMB DRY-rewire**: estimateShipping → 1-line delegate to the shared module, perBoxForCategory removed (−101 lines); regression-gated LIVE (still $16.55/17lb/zone2, byte-identical). (5) **DTF `/invoice` Shipping row**: TotalAmount now EXCLUDES shipping + a SHIP fee item (tax still on base+shipping) → `/invoice` shows a Shipping line + foots ($143.13, no double-count) — VERIFIED LIVE (DTF0608-3) + locked by `tests/unit/dtf-tax-base.test.js`. 1026 tests. (Test quotes DTF0608-2/-3 'POLISH TEST' left in Caspio — safe to delete.)
