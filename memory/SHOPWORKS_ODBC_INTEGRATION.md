...
## FULL current-state pipeline (discovered 2026-07-17) — consolidation target

- **Current**: a desktop logged in as user **FrontDesk** runs `OneDrive\ODBC Shopworks Scripts 2026\Run-All-NWCA-Exports.ps1` → 5 PowerShell ODBC exports → CSVs in the FrontDesk OneDrive root → Caspio scheduled imports. The 5: **SalesReps→Sales Reps 2026-Daily, Designs→Designs2026, Contacts→CustomerContactsMerge, PurchaseOrders→PurchaseOrders, Orders→Orders_ODBC**. Scripts are readable from Erik's laptop OneDrive (`C:\Users\erik\OneDrive - Northwest Custom Apparel\ODBC Shopworks Scripts 2026\`: NWCA_{SalesReps,Designs,Contacts,PurchaseOrders,Orders}_Export.ps1 + Run-All; `.creds` subfolder holds creds). Run-All is interactive (Read-Host at end).
- **Thumbnail images (separate flow)**: a desktop task copies `\\NCA-FS01\Thumbnails` → renames `{ID_Serial}_{name}` → `OneDrive\Thumbnails_Server_Caspio` (22,164 images, online-only) → uploaded to Caspio Art folder. RETIRE once bandit-direct image sync built.
- **CONSOLIDATION TARGET**: move ALL of it to bandit (always-on, already has ODBC + runs the order sync). End state: no FrontDesk desktop, no OneDrive — bandit → proxy/Caspio. Order of migration: (1)✅ Orders done; (2) other 4 metadata exports → bandit; (3) thumbnail image sync → bandit, retire Thumbnails_Server_Caspio; (4) thumbnail metadata → drop to `\\BANDIT\SWF` (stays manual until ShopWorks exposes Thumb). ⚠ Orders now double-updates Caspio (old CSV export in Run-All + new bandit sync = intended parallel-run; delete "Orders" from Run-All after trust window).
