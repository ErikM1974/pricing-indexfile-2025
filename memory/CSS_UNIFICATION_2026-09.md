...
- Still cosmetic-only (not restored): button variants, product-row and pricing-breakdown polish, quote-share modal internals, rich artwork upload, push-preview contents, legacy `qb-*` utility classes in static HTML, and the DTG catalog/chat card polish. Audit output: session scratchpad `orphans/functional-full.json` and the workflow journal.
