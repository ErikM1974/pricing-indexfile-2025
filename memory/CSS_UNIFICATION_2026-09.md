...
## Active checkpoint — lead records released, personalization workflows next

Standing edit/deploy authorization continues. **Frontend v2026.09.09.20 / Heroku 2096 / ddfc539528ccafd5c6adf12f94513a9a4bcbde0b is LIVE.** Successful release and running slug verified 2026-09-10T05:34:06.0289029Z. Twenty-two live checks passed: four versioned sources and four content-hashed copies match, seven prior access checks preserve authentication/session behavior, four retired stylesheets return404 and three public pages remain200. API version matches the full release SHA. No business writes, sends, uploads or notifications.

**158 of225 application pages reviewed and live;67 pending.** Another79 HTML sources are excluded:12archive,14email,50fixtures/templates,3retired. All26training pages complete. Shared runtime forms, generated documents and external-provider UI remain separate unfinished tracks. Backend unchanged: v2026.09.08.1 / Heroku1130 / d06aee3e4d25c5e1410241ea8007cdc8339aa3fa. Application-wide cleanup is not complete.

### Lead records completed

- Three pages: lead detail, Forms Inbox and Marketing Shipments. Canonical crm-records.css with existing components/pipeline, Public Sans/navy-gold, consistent controls, responsive cards and tables, keyboard access and readable paper output. Four old sheets/42,003LFbytes/four important flags retired; new owner zero flags. Three static preview consumers migrated with strict API stubs.
- Reviewed source02af336bb51431d33152d6a2ff9712557f5481a0 integrated at8be8ee84ecdf123ca8d7335440859897b842998d. Exact source c8523b2655fd1dadb2bdcf678309dc882850ba1e passed CI 34440499535 with all four jobs and actual mocked-browser/live-money-calculator steps. Main CI 34441502741 passed; expected browser skip on changelog commit. One Heroku push.
- Full local gates:232unit suites/5,383passed/four existing skips;88DOM,four accessibility unit,84quote parity,679mocked browser,16remaining browser/three optional screenshots skipped,all five calculators,276CSS clean. Build/lint/types/boot,485routes/24modules/zero undefined names,production audit zero.
- 25focused cases,275source/CSS/historical checks,17original hashes/93reversible controller edits plus HTML/retirement provenance. Native Inbox dialog focus, stale lead/list/activity/quote responses, valid current quote value sync, kit/art lifetimes, original sample and four quote handoffs, exact failure payloads and uncertain outreach wording covered. Five PDF outputs/seven reviewed pages preserve populated orders/amounts/statuses and complete details.
- Direct CSS graphs129,469LFbytes each, down from186,763/167,810/177,211. Lead art-trigger graph165,253bytes is counted separately. Original garment-submit-form.css/JS remain byte locked and pending shared-module styling; its lead prefill/focus/close trigger is reviewed. LESSONS_LEARNED255lines.

### Next: four personalization consumers

- Isolated path C:\Users\erik\.codex\visualizations\2026\09\07\01a07d90-9a4c-7e70-9e4e-c196377b7c6b\training-final-worktree, branch codex/personalization-workflows, plan ab95e14ccb70c682a7c91d876e641957f713a0b1, based on reviewed lead source 02af336bb51431d33152d6a2ff9712557f5481a0. Candidate v2026.09.09.21. Four pages: monogram dashboard, names-and-numbers dashboard, names-and-numbers entry and monogram form. Previous dashboard-only draft was committed at d0456e0795284b02cbb5abb379f986c9419f4e1d and backed up. Twenty-one source checks, twelve focused browser cases and two visually reviewed landscape PDFs passed; both four-width cases passed again after print cleanup. Nineteen controller edits preserve source provenance and fix stale-list/retry races. Entry forms and old CSS owners remain unchanged. No migration credit yet.
- Twenty-one original HTML/controller/helper/style hashes, four-width populated original captures and original PDFs saved with fully mocked APIs. Narrow roster/monogram forms overflow320px by32/22px. Additional monogram baseline uses supported Left Sleeve/Right Chest values; original unsupported Left Chest fixture is retained as separate evidence. Existing normal-style reload appears to omit style numbers from the manual input; verify and lock the actual data before fixing.
- Preserve names, leading-zero jersey numbers, garment groups/quantities, font samples, authoritative thread colors, location choices, QA and service payloads. Capture import/dialog/error/retry/focus/print/export states and every shared-owner consumer before retiring styles. Never send real OCR/uploads/saves/deletes or messages.
- Finish isolated review, record exact reviewed SHA, integrate only that SHA with current primary memory, then full local checks and actual exact-source CI before one verified release. Never merge an advancing worktree blindly.

### Resume evidence and safeguards

Private artifact directory: C:\Users\erik\.codex\visualizations\2026\09\07\01a07d90-9a4c-7e70-9e4e-c196377b7c6b. Current release evidence: lead-records-{reviewed-source,integration-record,source-record,source-ci,release-record,local-gates,heroku-verification,live-verification,main-ci,checkpoint-record,review-record,paper-review,populated-paper}.json and pricing-lead-records-* logs. Next-family personalization-{plan,original-baseline,original-capture,review-data}.json and personalization-valid-monogram-* plus original source/screenshot/PDF copies. active-css-resume-state.json is current; older shutdown files are history.

Completed mutation/commit/release helpers are one-shot; never rerun them. Original captures are complete and must not be recaptured after edits. Isolated node_modules is a junction into primary: never recursively delete through it; no .env copied. Stage explicit owned paths only. Backend preexisting untracked .agents/ and AGENTS.md untouched.


### Personalization draft in progress — 2026-09-09

- Three layout drafts in the isolated codex/personalization-workflows checkout: monogram dashboard, names-and-numbers dashboard and names-and-numbers entry. Monogram entry still uses its original visual layout; its saved-style restoration bug is now fixed and tested. Old CSS owners remain present. No additional pages registered or deployed.
- Eighteen focused browser cases and twenty-two source/HTML preservation checks pass. Seventy-eight reversible controller mappings retain the original twenty-one hashes plus the newly adopted, unchanged ui-dialog.js helper. New CSS has zero important flags and undefined tokens; both edited form controllers pass ESLint.
- Roster entry uses canonical controls, keyboard-scrollable editable columns, collapsed group settings and native dialogs wrapped by the shared focus/scroll helper. All four widths retain original names, leading-zero jersey numbers and garment quantities. Three dialog lifecycles pass focus/escape/accessibility checks.
- Roster paper keeps the existing active-group scope and size-breakdown semantics. Two outputs/seven pages visually reviewed: one normal page with all garment/custom columns, plus six wide-example pages retaining all288 distinct custom values and repeated headings. Two dashboard PDFs/two pages are also visually reviewed. Source and print checks do not authorize or perform real business writes.
- Remaining before review completion: roster configuration/import/OCR/search/save/retry/stale-response/export paths; monogram layout, focus/recovery, production/proof output and special font/thread-color behavior. Preserve service and QA helpers. Register the four-page family and retire old owners only when every consumer is covered, then integrate an exact reviewed commit and run full local gates plus exact-source CI.
- Evidence in the private artifact directory: personalization-roster-combined-browser.log (18), personalization-roster-source.log (22), personalization-roster-paper-review.json (seven pages and extracted values), personalization-style-restore-before.log (proven defects), personalization-style-restore-final.log (fixed), controller/style lint logs and original source/capture files. Latest committed draft SHA is recorded in personalization-draft-record.json and active-css-resume-state.json. Original captures must never be rerun.
...
## CRM and lead workspaces plan — candidate v2026.09.09.19

Six related pages share the next batch: dashboards/house-accounts.html, dashboards/nika-crm.html, dashboards/taneisha-crm.html, dashboards/leads.html, dashboards/lead-scorecard.html, dashboards/unqualified-leads.html. Preserve CRM navy/gold identity, account/lead figures, filters, source URLs, archive semantics and all API/payload rules. Use canonical tables, controls, typography and focus with separate scoped account and pipeline layout owners. Retire old CSS only after its last consumer migrates.
The detail workspace dashboards/lead.html and its dynamically loaded garment-submission form stay a separate follow-on. Keep dashboards/css/leads.css for that remaining consumer; its HTML, controller and original stylesheet are source locked. No cross-family garment-form change is included here.

- [ ] Capture original synthetic account, lead, scorecard, filters, dialogs, CSV and paper outputs at four widths.
- [ ] Consolidate layout owners and wire native accessible controls without changing data contracts.
- [ ] Verify complete values, failed/empty/partial/retry states, keyboard navigation, mock mutation payloads and print.
- [ ] Integrate full local checks, exact-source CI and verified deployment before credit.

Live frontend is v2026.09.09.17/Heroku2093/d5b544d1375d5b143ccbe3666a8c84ca5b6410e9 (146of225/79pending). Pricing reports .18 is reviewed at75835086c06c3d4a0ac41876ab021c6ad050f85d and integrated at8e8d5496bf668961455d42babf22a7dd195e1786; full local gates are running. This plan adds no review/live credit. Mock every API request, including GET reconcile?autoAdd=true and sync actions that can mutate. No actual business writes, imports, uploads, email or notifications. Preserve the node_modules junction and never copy.env.
...
Added tests/unit/crm-workspaces-content.test.js for the six CRM page contracts and17source hashes; verification pending.
Registered tests/e2e/css-unification-crm-workspaces.spec.js for all six CRM layouts and interactions with synthetic services; no live business writes.
...
## Lead records draft checkpoint — continue focused review

Primary CRM release v2026.09.09.19 / Heroku 2095 / d85755456dba796544316a44ea5f7b3050db270e is verified live; primary checkpoint 4a205d1a599f5cf9adfd87a9081db2901a119e60 is pushed and CI green. Live155/225,70pending. This isolated three-page candidate2026.09.09.20 remains unreviewed and undeployed; no count credit.

Lead detail, Forms Inbox and Marketing Shipments now use tokens/components/crm-pipeline plus scoped crm-records.css. Three original controllers have84 explicitly mapped UI/recovery changes. Fourteen original script/style/helper hashes remain reconstructible and all three HTML content contracts hold; 17 content/source tests pass. Twelve focused browser cases pass, plus four responsive/print cases rechecked after paper cleanup. No baseline overflow or axe findings at1440/768/390/320. Lead paper3→2pages, inbox detail2→1, list/shipments1each; allfivepages reviewed. Status, assigned rep, contact, amount, samples and tracking remain legible. Native inbox dialog cyclesfocus, hides its backdrop on print, restorestrigger on close. Error banners remain in page flow.

Still required before final review: expanded lead edit/kit/outreach/quote/customer/art-form states using mocks; loading/empty/malformed/stale read races for inbox/detail/shipments; current source and shared runtime-form boundary checks; full data-word assertions for populated paper; synchronize static preview consumers and supersede historical excluded-lead source locks with explicit new evidence. Keep four legacy CSS owners until all consumers migrate. Register only afterreview, update lifecycle/lessons/counts, integrate exact reviewed SHA into currentprimary, fullgates/exact-sourceCIactualbrowser-money, oneverifiedrollout. No business sends/uploads/imports/returns/shipment writes.

Evidence in taskartifactdirectory: lead-details-{plan,original-baseline,original-capture,review-data,controller-changes,draft-capture}.json; lead-records-{source-tests,browser-followup,print-final}.log; lead-records-paper-review.json and renderedfivepageproof. Originalcapturehelpers and layout/recovery/commit helpers are one-shot; do not repeat. Isolatednode_modules remainsjunction intoprimary; never recursively delete through it orcopy.env.
...
Personalization draft: added shared_components/css/personalization-workspaces.css and migrated only the two list dashboard HTML/controller consumers to canonical controls. 7 generated control-class changes are reversible in the private personalization-controller-changes.json. Forms and four old owners remain unchanged while the full four-page family is under review. No retirement, registry or live credit yet. Original21file hashes and source captures retained.

## Personalization dashboard draft checkpoint

The two list dashboards use the new scoped personalization-workspaces.css with canonical controls, Public Sans and navy/gold shell. Nineteen reversible controller edits include seven control-class changes and twelve loading/validation/latest-response guards. Twenty-one original source/HTML checks pass; twelve focused browser cases pass across four widths with zero fixture axe violations, complete rows/fields, keyboard scrolling, printed headings, filters, failure/retry, missing-list validation and superseded-response recovery. Original roster and monogram entry forms, their service/QA helpers and all four legacy CSS files remain unchanged. No registry or live credit; finish all four planned consumers and full paper/export/interaction review before release.

Also mirrors the released-source candidate lead-record timezone fix from primaryc8523b2655fd1dadb2bdcf678309dc882850ba1e; .20 CI34440499535 is running after the original UTC-versus-Pacific comparison failure. LIVE remains.19/2095/155reviewed/70pending. Prepared next sources never enter that exact-source CI or deployment. Private personalization-original-review-notes.md records original form overflow, clipped roster paper and missing monogram style numbers; fix with tests. Original and supported-location monogram captures are separate and immutable.

### Personalization dashboard print review — 2026-09-09

Both populated dashboard PDFs have been visually reviewed after removing the full-page gray background and duplicate monogram heading. Each fits one landscape letter page with all synthetic orders, companies, statuses and totals. Both four-width browser/accessibility cases passed again on the final print styles. Entry forms remain unchanged and pending.
