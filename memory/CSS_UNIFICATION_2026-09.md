...
## Active checkpoint — CRM workspaces released, lead records next

Standing edit/deploy authorization continues. **Frontend v2026.09.09.19 / Heroku 2095 / d85755456dba796544316a44ea5f7b3050db270e is LIVE.** Successful release and running slug verified 2026-09-10T03:54:22.7105415Z. Thirty-two live checks passed: seven versioned sources and seven content-hashed copies match, ten prior access checks preserve authentication, four retired stylesheets return 404, three public pages and the still-required leads.css return 200. API version matches the full release SHA. No business writes, sends, uploads or notifications.

**155 of 225 application pages reviewed and live; 70 pending.** Another 79 HTML sources are excluded: 12 archive, 14 email, 50 fixtures/templates, 3 retired. All 26 training pages complete. Generated documents and external-provider UI remain separate unfinished tracks. Backend unchanged: v2026.09.08.1 / Heroku 1130 / d06aee3e4d25c5e1410241ea8007cdc8339aa3fa. Application-wide cleanup is not complete.

### CRM batch completed

- Six pages: House Accounts, Nika CRM, Taneisha CRM, Leads, Lead Scorecard and Unqualified Leads. Two canonical CSS owners: crm-accounts.css and crm-pipeline.css. Navy/gold, Public Sans, consistent native controls, keyboard access, responsive cards/tables and complete print output. Four old sheets and seven important flags retired. Old leads.css remains for lead detail and Marketing Shipments.
- Reviewed source d44f7551fc1d3a251c12599314f8c23b94fadea4 integrated at 8ed0ea00a871dc047f71b66c52a8c766516a8561. Exact source 37fe944b6ceb2bbdb4361a37aaefaaee61d0a53d passed CI 34433719156 with all four jobs and actual mocked-browser/live-money-calculator steps. Main CI 34435013705 passed; expected browser skip on changelog commit. One Heroku push.
- Full local gates: 231 unit suites, 5,359 passing tests and four existing skips; 88 DOM, four accessibility unit, 84 quote parity, 654 mocked browser, 16 remaining browser and three optional screenshot skips, all five calculators, 279 CSS clean. Build/lint/types/boot, 485 routes and 24 modules with zero undefined names, production audit zero.
- Focused evidence: 31 CRM browser cases; 11 source/CSS/historical suites with 299 checks. 88 explicitly mapped UI/recovery changes reverse 17 original source hashes. Original card, table, rep-detail and CSV values preserved. Six base PDFs/eight pages preserve 640 original data words; board print adds two complete pages with all five companies. All ten paper pages and four responsive widths reviewed.
- Recovery fixes reject malformed data, clear stale totals, keep failed view switches recoverable and prevent late responses restoring old state. Native dialogs retain focus and show failures inside the active dialog. House row disclosures use native buttons. Valid pricing, CRM math, payloads and background fallback behavior preserved.

### Next: lead records, Forms Inbox and Marketing Shipments

- Isolated checkout C:\Users\erik\.codex\visualizations\2026\09\07\01a07d90-9a4c-7e70-9e4e-c196377b7c6b\training-final-worktree, branch codex/lead-details. Plan c1e47fca0e07a2a51574c199cdc72260726bec0f based on d44f7551fc1d3a251c12599314f8c23b94fadea4. Candidate v2026.09.09.20. Three pages: dashboards/lead.html, dashboards/form-submissions.html and dashboards/marketing-shipments.html. Original source capture complete; isolated review complete with 93 mapped UI/recovery edits; production credit waits for verified release.
- Seventeen original source hashes plus HTML contracts, four-width synthetic captures, tables, lead activity, sample-item detail dialog, requested/packed/shipped rows and original PDFs saved. No errors, missing assets, unknown API calls or writes during original captures. Do not recapture originals after editing.
- Reuse canonical components and CRM pipeline atoms, add scoped record layouts. Preserve lead statuses, money, contacts, quotes, attachments, outreach payloads, sample returns and tracking. Mock every API and action, including mutating GETs; never send real outreach or shipment updates. Review loading/malformed/empty/error/retry, stale responses, keyboard/dialogs, original content and full print before registering.
- Keep dynamically loaded garment-submit-form.css/JS and quote-view consumers outside the styling boundary unless all consumers are tested. Supersede previous CRM excluded-source locks only with explicit mapped changes and new preservation evidence. Migrate every consumer and preview before retiring leads.css, lead-workspace.css, form-submissions.css or marketing-shipments.css.
- Finish isolated review, record exact reviewed SHA, integrate only that SHA with current primary memory, run full local gates and exact-source CI actual browser/money steps, then one verified release. Never merge an advancing worktree blindly.

### Resume evidence and safeguards

Private artifact directory: C:\Users\erik\.codex\visualizations\2026\09\07\01a07d90-9a4c-7e70-9e4e-c196377b7c6b. CRM crm-workspaces-{reviewed-source,integration-record,source-record,source-ci,release-record,local-gates,heroku-verification,live-verification,main-ci,checkpoint-record,paper-review,review-record}.json and pricing-crm-workspaces-* logs. Next-family lead-details-{plan,original-baseline,original-capture,review-data}.json plus original source copies and screenshots/PDFs. active-css-resume-state.json is the current task pointer; older shutdown files are history.

Completed mutation/commit/release helpers are one-shot; never rerun them. Original capture helpers have completed. Isolated node_modules is a junction into primary: never recursively delete through it; no .env copied. Stage explicit owned paths only. Backend preexisting untracked .agents/ and AGENTS.md are untouched.
