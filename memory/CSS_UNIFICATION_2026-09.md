...
Erik resumed; standing edit/deploy authorization continues. **Frontend v2026.09.08.16 / Heroku 2075 is LIVE**, full SHA **520acd1d6c9bb9acba0fa4e4612247d632d98c66**, actual slug verified 2026-09-09T04:29:59Z. Exact-source CI **34310205668** passed all four jobs, including live money/calculator parity, on **76a52b66f0de2a049e639a6bc446cd7163e19275**. Nine deployed runtime assets match committed bytes; four training HTML shells and two public form regression shells return 200; four staff shells remain protected. No business writes or notifications. Main/tag/develop were synchronized; subsequent documentation commits are expected. Track follow-up main/develop CI separately.

**60 reviewed application pages live; 165 pending**, from **225 application/served-archive HTML sources**. Another 79 tracked HTML sources are excluded. **All 26 tracked training pages are reviewed/live.** Ignored local training/api-test-runner.html is not tracked/deployed and is excluded; leave it alone. Generated document/style owners remain separate unfinished work.

### Next batch and plan

- [x] Unify/deploy Training Center, sales-tax-code trainer and both ShopWorks customer-setup exercises (.16).
- [x] Preserve original course data, prose, links, fields and options; verify four widths, keyboard/native dialogs, full and restarted rounds, scores, timers, storage failures and complete active-view printing.
- [ ] Finish the four API reference dashboards: Caspio, ManageOrders, SanMar and ShopWorks ODBC. Separate worktree **C:/Users/erik/.codex/visualizations/2026/09/07/01a07d90-9a4c-7e70-9e4e-c196377b7c6b/training-final-worktree**, branch **codex/api-reference-family**, base 76a52b66. Plan commit 23f3dec2. It contains uncommitted application work belonging to this task; do not delete/reset it. node_modules is a junction to the primary frontend. No .env copied.
- [ ] API reference baselines are saved; common layout implemented locally in api-reference.css and four page owners. Controls, original-data locks, registration/testing/docs, desktop/phone/print review and full release gates remain. This API reference batch is NOT ready/live.
- [ ] Integrate the reviewed worktree into current develop, preserving .16 release/documentation changes; exact-source CI and verified deployment, then remaining application families.

Design: Public Sans, white/neutral/forest tokens and shared controls. API references replace art-hub/dash-shell dependencies; preserve all technical data and access gates, keep endpoint descriptions visible on phones, add keyboard catalogue disclosure/scrolling, labeled search/counts and useful ODBC failure/retry. No business-service calls during browser review.

### Verification / resume evidence

.16: **208 unit suites / 5,066 passed / four existing skips**, 88 DOM, four a11y unit, 84 quote parity; full local **209 mocked browser plus 16 remaining browser cases**, three optional screenshot skips, all five live calculator surfaces. Four new PDF cases followed that full browser run; final focused family suite **21 passed**, ownership/content **121 passed**, deterministic suite repeated after print fixes. Exact-source CI passed all final checks together. Build/lint/types/**294 CSS**/HTTP boot pass; 485 routes / 24 modules unchanged. Reviewed active-view PDFs: directory 2 pages, tax 1, basic setup 1, enhanced setup 2; screen values restored after print. Unique family CSS source **47,589 to 37,439 bytes (21% less)** including print contract; not network transfer.

Artifacts: C:/Users/erik/.codex/visualizations/2026/09/07/01a07d90-9a4c-7e70-9e4e-c196377b7c6b. training-final-{release-record,source-ci,heroku-verification,live-verification,local-gates}.json are authoritative. api-reference-original-baseline.json records the next four originals at 76a52b66. Do not rerun one-shot installers/release helpers. Heroku push helper is single attempt: inspect actual release SHA/status after a disconnect before retrying.
