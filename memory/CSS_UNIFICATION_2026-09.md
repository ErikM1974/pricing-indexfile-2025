...
Erik resumed; standing edit/deploy authorization continues. **Frontend v2026.09.08.18 / Heroku 2077 is LIVE**, full SHA **9c82a4991ee2b2581187299fd655bba986876015**, actual slug verified 2026-09-09T05:52:31.1709480Z. Exact-source CI **34315635746** passed all four jobs, including live money/calculator parity, on **c0c8dd25cb41df08510289c4c16365512db3273f**. Five deployed runtime assets match committed bytes; the policy migration tracker and four other staff shells remain protected; nine public/new/regression shells return 200. No business writes or notifications. Main/tag/develop were synchronized; subsequent documentation commits are expected. Track follow-up main/develop CI separately.

**68 reviewed application pages live; 157 pending**, from **225 application/served-archive HTML sources**. Another 79 tracked HTML sources are excluded. **All 26 tracked training pages are reviewed/live.** Ignored local training/api-test-runner.html is not tracked/deployed and is excluded; leave it alone. Generated document/style owners remain separate unfinished work.

### Next batch and plan

- [x] Unify/deploy policy migration tracker, historical pricing-negotiation guide, Resources and Sale (.18), following the four API dashboards (.17).
- [x] Preserve historical policy prose/figures and the exact 414-row tracker snapshot; verify actual role restrictions, four widths, keyboard filters/scrolling, visible malformed/offline/empty retry, native contents/history/focus and complete PDFs.
- [ ] Finish Policies Hub, Policy Detail, Open Questions and Handbook together. Separate worktree **C:/Users/erik/.codex/visualizations/2026/09/07/01a07d90-9a4c-7e70-9e4e-c196377b7c6b/training-final-worktree**, branch **codex/policy-cms-family**, base **10a439b32c7e0b80ba500153284795f894c2dac5**. It contains uncommitted application work belonging to this task; do not delete/reset it. node_modules is a junction to the primary frontend. No .env copied.
- [x] CMS baseline saved; old policies-hub-v2.css renamed to policy-workspace.css and all four pages moved onto shared tokens/components/Public Sans. Scoped detail/handbook/org-chart arrangements retained. Native contents, shared read-only table scrolling, responsive header and view/retry/focus fixes implemented; API client/auth gate/organization data unchanged and locked.
- [ ] Initial CMS **13 focused browser cases passed** (four widths, axe, storage/search/retry, staff visibility, handbook chapter failure/retry/history, native AI dialog and PDFs); **137 content/ownership/auth guard cases pass**, one sibling-proxy check skipped in isolated worktree. Actual populated paper review, real editing/comment/moderation/chart states, remaining lint/registration refinements and full release gates remain. Candidate manifest 72 is NOT a live count. No CMS policy writes have been sent.
- [ ] Integrate reviewed CMS work preserving .18 documentation, pass full local checks and exact-source CI, verify actual deployment, then remaining families.

Design: Public Sans, white/neutral/forest tokens, shared controls, domain-specific reading/editor/chart layouts and a separate print contract. Preserve policy text and actual permissions. All business-service calls in browser review are mocked.

### Verification / resume evidence

.18: **210 unit suites /5,088 passed /four existing skips**, 88 DOM, four accessibility unit, 84 quote parity; **244 mocked browser (364.6 seconds) plus 16 remaining browser cases (4.1 minutes)**, three optional screenshot skips. All five live calculator surfaces, including EMB/CAP additional-logo tiers, match. Exact-source CI passed all final checks. Build/lint/types/**295 local CSS**/HTTP boot pass; 485 routes /24 modules unchanged. Fourteen focused family browser cases pass. Actual PDF review: tracker 33 pages, all 797 checked unique record fields plus six static blocks; guide 11 pages/all 161 checked content blocks/headings; Resources/Sale one page each. All 414 tracker rows print and screen filters are restored. Tracker CSS graph shrinks about 62%; guide/notices add cached shared controls. Raw source counts exclude external Bootstrap, so do not claim a network reduction.

Artifacts: C:/Users/erik/.codex/visualizations/2026/09/07/01a07d90-9a4c-7e70-9e4e-c196377b7c6b. policy-reference-{release-record,source-ci,heroku-verification,live-verification,local-gates}.json are authoritative. policy-cms-original-baseline.json records the next four originals; policy-cms-isolated.config.cjs uses port3419. Do not rerun one-shot installers/release helpers. Inspect actual Heroku SHA/status after a disconnect before retrying a push.

Backend unchanged: v2026.09.08.1 / Heroku1130 /d06aee3e4d25c5e1410241ea8007cdc8339aa3fa. Dependency/runtime backlog and natural scheduler verification remain separate. Frontend production audit zero.
