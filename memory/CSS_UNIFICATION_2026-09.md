...
### Current isolated draft — first two toolkit pages

Contract Break-Even and Roland Supplies now use tokens.css, components.css and the draft staff-toolkit.css owner. Direct CSS is 65,002 LF bytes each (previously 166,499 / 164,993). Zero important flags and undefined tokens. No old CSS retired and no migration credit yet; six other toolkit pages are unchanged.
Thirty-one original source contracts and 258 source/CSS guards pass. Thirteen reversible edits: nine HTML shell/font/link updates and four contract controller edits. Contract values match every original table in all four cost/profit and typical/worst modes. The controller now disables print/toggles while loading or invalid, reports incomplete rates and offers a real Retry. Nine focused browser scenarios have passed (eight pair cases plus one immutable original-volume scenario). Four-width captures and axe pass for both draft pages; the Jotform provider itself is mocked and excluded from the host-page axe scan. Its original embed URL and printable PDF link remain unchanged.
Two contract PDFs / six pages were reviewed after readable print spacing, page margins and title were added. Customer/memo Volume Quote originals and all three synthetic save requests (including the allocating GET) are preserved in tests/fixtures/staff-toolkit-volume-original-workflow.json and private PDF artifacts. Volume Quote itself is still unchanged.
Next: migrate Volume Quote, then the other five toolkit pages. Preserve fixed-width memo formatting, all original cost/price/payload contracts and customer/internal print separation. Investigate stale style/stock responses, editing during asynchronous save, partial-save wording and data-failure states with mocked behavioral tests. Existing shared legacy owners and every page-specific old sheet stay until consumer retirement review. Full application gates and exact-source CI still required before deployment.
