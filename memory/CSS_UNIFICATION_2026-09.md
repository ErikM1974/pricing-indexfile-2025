...
## Staff toolkit release record — v2026.09.10.2

Exact-source CI 34469904476 passed all four jobs on ca46b471f92494a10bf16db76f47698f5e7e82c5, including actual live money/calculator parity and 813 mocked browser checks. Eight staff pages plus the Jim preview share canonical controls and one scoped owner. There are 77 focused scenarios, 33 original hashes, 384 reversible changes, and 29 reviewed paper pages. Eight old sheets / 82,193 LF bytes / 16 flags retired; 485 routes unchanged. Count becomes 170 reviewed / 55 pending only after verified rollout. Six more staff pages have a separate source-only baseline on codex/staff-workspaces; that branch is excluded from this release.
...
### Payroll baseline underway — 2026-09-10
Original synthetic workflow passes before visual edits. Existing pay-period selector overflows phones (401px at 390/320); upload reaches321px at320. Seven synthetic employees preserve five printable slips plus one blank slot, two blocked records, original carryover/sick values, audit CSV and both reviewed import payloads. No actual API writes. Live remains170/225 (.2); two reviewed workspace drafts are separate. Latest draft804be4a is local; GitHub TLS connectivity has delayed remote confirmation.
...
## Calculator reference release record — v2026.09.11.4

CI 34597016238 passed all four actual jobs on 911f21ddea15963000b156da7fbe1d0ae9bd7f87, including CSS and live money/calculator parity. Full local checks passed on the identical complete source tree e57cc4565142166532f7932d95bbd0b1ea5c73aa at 5caf8bbb9aaa4c7759c6196de41102ba79dbc51f; the empty Deploy marker only requested the required CI job. Manual Pricing, Manual Laser and Shop Menu use canonical controls and scoped style owners. Sixteen focused browser cases,23 visually reviewed current paper pages,19 original source hashes,18 original browser contracts and35 reversible mappings preserve behavior. The menu controller is unchanged; the manual controller differs only by two literal accessibility additions. Keyboard tables retain every size/price, failed pricing remains visible and normal menus print on one page. Counts become199/225 reviewed,26pending after verified rollout. Core calculator baseline work remains on its separate branch and is excluded.
...
## Core calculator release record — v2026.09.11.5

CI 34620075822 passed all four actual jobs on 62b3ad7acc48cdc002619675f2a8bc9390f3fdb0, including CSS and live money/calculator parity. Full local checks passed on the identical complete source tree 9ab9e9e903426845fba201ba2c4680d4eccf2eb2. DTG, DTF, embroidery, cap embroidery and screen printing now share Public Sans, canonical controls and core calculator layout with small scoped page sheets. Fifty focused browser cases and the full regression suite cover preserved prices, URL inputs, inventory, keyboard controls, accessible errors and phone layouts. All36 current paper pages across20 documents were visually reviewed;114 reversible mappings preserve the original source evidence. Six financial services and31 financial function bodies remain unchanged. Counts become204/225 reviewed live,21pending after verified rollout. Contract calculator work remains in its isolated branch and is excluded.
...
**LIVE v2026.09.16.5 (Heroku v2128, main c4e582bb).** Exact-source CI green. Note: `CAPTURE_QUOTE_BUILDERS_ORIGINAL=1` failed ~30 states for reasons unrelated to CSS (random artwork-upload ids, the fixtures' `localhost:3414` share URL, toast timing, contrast readings taken under the pointer, mid-fade or mid-lookup), on the pre-change commit too. Since 2026-09-17 it passes on any port: `stableOriginal()` in `tests/e2e/helpers/quote-builder-workflow-review.js` normalizes those samples on both sides without regenerating a fixture (LESSONS_LEARNED, "Saved browser evidence must not record timing samples"). The Quick Quote capture mode passes.
...
- Screen Print: minimum-order banner (now `class="workspace-min-order"`, same as DTF). DTF: its own toast. DTG: out-of-stock dot and input, avoid-colour swatch, rank badges (and the top-seller grid's third column), colour warning box.
- Locks: `tests/e2e/css-unification-quote-builder-workflows.spec.js` (computed-style probe on all four builders; vendor import by keyboard), `tests/dom/emb-import-summary.test.js`.
- The cosmetic-only groups the same audit found were restored on September 17 (next section).

## Builder cosmetic states restored — September 17, 2026

**LIVE v2026.09.17.1 (Heroku v2129, main 03b96f59).** Exact-source CI green on the Deploy commit 820eb9e5 (including Playwright); the five changed stylesheets served by production are byte-identical to the release. Live staff pages need a signed-in browser — Erik signs in himself (see auto-memory `feedback-staff-login-live-checks`).

The 52 cosmetic-only groups from the September 16 audit (28 Embroidery, 24 Screen Print/DTF/DTG) now have token-based rules, so no builder state falls back to plain text. Old rules were the guide for intent; colours are the family's AA pairs (`--color-*-ink` on `--color-*-bg`, `--ui-accent` on `--ui-tint`), never the old low-contrast greys or white-on-amber pills.
- Shared (`quote-workspace.css`): success/info toasts (`showToast` emits `toast-success`, not `toast success`); primary Push to ShopWorks / preview confirm / DTF Apply / share-dialog actions with matching `:hover:not(:disabled)`; email, rush, estimate, tax-lookup, logo-refresh and ghost row buttons; product-row states (new row, readonly description, child and different-colour rows, available sizes, OSFA, size labels, empty swatch); small-order panel and tier nudge; ship-to card; customer-lookup results; share-dialog internals; rich artwork upload; push-preview contents (`qb-grid-2col`, `qb-table-13`, `qb-warn-box`…); size-popup loading/empty; the legacy `qb-*` hooks Screen Print, DTF and Embroidery still carry in static markup; DTF step badges, empty-table message and row hover; the Screen Print/DTF keyboard hint (moved from `quote-dtf.css`). On paper, the available-size and new-row tints are removed.
- Embroidery (`quote-embroidery.css`): logo-card design lookup, stitch-estimate button beside its select, AL digitizing, design-search tags and tier badges, import preview/progress bar (the bar was 0px tall), push preview, service pricing review, shipping modal, footer order details and notes, empty-state step pills, small-order notice container, pricing breakdown, overridden price, vendor/cap badges, Enter manually, assistant markdown (the bubble is `white-space:normal` because the renderer already emits `<br>`).
- DTG (`quote-dtg.css`): sticky form column above 1200px, catalog and full-SanMar results (results span the grid), order-form polish, submit status and stock-confirm buttons, resume banner, history pill, new-artwork cards, assistant cards, CTA. The recurring axe contrast failure on `.dtg-cc-add-default` was a hover-specificity bug (LESSONS 2026-09-17), fixed here.
- `specialty-assistant.css`: sparkle glow, Ask label, primary assistant action, left-aligned assistant messages.
- Deliberately left: the SPR modal's header gradient hook and other hooks the family already covers (`qb-field*`, `qb-maxw-760`, `qb-p-16`, the dashed/cyan inputs), `.extended-col` (the family keeps one header style), `.fee-summary-section` (tbody borders don't render in the separated table model), `.dff-pricing-link` (removing its underline would fail link-in-text contrast), and assistant/DTG details the agent listed as covered.
- Budgets: raw CSS for Embroidery 264,084 → 315,770 bytes, Screen Print 242,140 → 262,457, DTF 241,294 → 261,371, DTG 288,795 → 319,409 (all still below their pre-migration baselines). Most of the growth is the required scope prefix on each rule.
- Text-capture note: rules restore the old `text-transform` only where the retired sheet had it (ship-to title, preview labels, SPR section labels, placement tags, vendor badges, footer order-details head), so captured text can only move closer to the original fixtures.
- Locks: `css-unification-quote-builder-workflows.spec.js` "cosmetic runtime states are styled" (fails on the September 16 CSS, passes now) plus the existing workflow evidence, axe and overflow checks: 69 Embroidery/Screen Print/DTF and 23 DTG/alternate browser tests pass.
