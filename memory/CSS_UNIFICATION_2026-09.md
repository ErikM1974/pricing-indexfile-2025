...
## ACTIVE — four staff workspace drafts reviewed, 2026-09-10

Current work is in the isolated training-final-worktree on codex/staff-workspaces. Read artifact active-css-resume-state.json for the exact reviewed commit and backup status. Primary develop and the completed .2 release remain unchanged; no new deploy.

- LIVE: v2026.09.10.2 / Heroku 2098 / df7bebed30da524464df4db1c40904191285f31d; 170/225 pages live, 55 pending. Backend unchanged .1 / Heroku 1130. Four additional reviewed drafts: Drain-Pro, production shifts, payroll and Mission Control; candidate 174/225, 51 pending, not live credit.
- Mission Control: Public Sans, navy/gold staff shell, AE accent, shared controls and six responsive tabs; native kit/outreach/full-list/inbound dialogs. Three new scoped owners: staff-mission-control.css, vendor-inbound.css and vendor-box-labels.css, plus existing vendor-invoice.css. Both app and manual harness migrated; all 12 shared harness regions synchronized. One legacy AE stylesheet retired; legacy inbound/label owners retained for unmigrated consumers. CSS graph 216,246 LF bytes versus original 263,878 (six versus seven local sheets), zero important flags and undefined tokens in the new owners.
- Reviewed original/current source-preserving browser scenarios: 16 passed, including all six tabs at 1440/768/390/320, rep/admin/degraded views, drawers, read-only logging, exact mocked kit/outreach payloads, native focus/error feedback, delayed identity, canceled tab initialization and obsolete previews. Twelve focused suites / 535 checks passed; affected controller ESLint and shared-harness check clean. Source fixture: 61 hashes and 160 reversible mappings, including shared-consumer asset versions; purchasing fixture also reverses the shared invoice changes.
- Ten current document outputs / 21 pages visually reviewed: invoice (1), full inbound (2), Aster/Cedar (1 each), both AEs (2), receiving/production/purchasing (1 each), box labels (8), call list (3). Original/current normalized PDF content matches; each label retains its own box number, WO, PO, design, tracking and size totals. Coherent paper fixtures include nonzero synthetic costs and received stock. Invoices now print on white paper; boxes and AE report sections use explicit page breaks. Generated call-sheet styling remains an explicitly separate modernization track, although its full content/pagination is preserved.
- Shared print cleanup owns its captured sheet and cancels stale timers. Lazy tabs mount only when their delayed loader runs; first inbound filtering follows identity resolution. Kit/outreach errors stay inside their native dialog; sends prevent duplicate actions and retain their result; outdated previews cannot overwrite a newer selection. Shared HTML consumers carry the current helper versions; no business API/payload changes or real sends/uploads.
- Next: Company Numbers original widget/error/paper baselines and redesign, then Quote Management using the new shared vendor owners. Full family integration, full application gates, actual exact-source CI and one verified release still pending. Preserve all figures, records and request bodies. Unknown API calls fail closed in browser tests; all business writes and allocating GETs are mocked.
- Never rerun one-shot migration or shutdown helpers. node_modules is a junction into primary: never recursively delete it; no .env copied. Stage explicit owned paths. Shared runtime, generated documents and external-provider UI remain unfinished tracks.

## Historical checkpoint — personalization released; staff toolkit next
