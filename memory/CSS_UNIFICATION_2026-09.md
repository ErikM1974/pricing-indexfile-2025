...
### Next: eight staff toolkit pages

- Worktree C:\Users\erik\.codex\visualizations\2026\09\07\01a07d90-9a4c-7e70-9e4e-c196377b7c6b\training-final-worktree, branch codex/staff-toolkit, checkpoint fa5c221c030e08cdcadf11b9a41f29b77d573ee2 (backed up to origin), based on personalization source b3147eab9220ef47330bc3a9a8c8d58287b3730a. Two draft page layouts are being reviewed; see the isolated-draft notes below. Thirty-one original HTML/CSS/JS hashes were verified against the previous live commit ddfc539528ccafd5c6adf12f94513a9a4bcbde0b and saved immutably. All 31 source contracts pass.
- Pages: dashboards/blog-editor.html, contract-break-even.html, customer-portal-admin.html, jim-mailing-list.html, past-due-orders.html, product-manager.html, roland-printer-supplies.html and volume-quote.html. Four-width original browser captures for all eight pages are complete with synthetic fixtures. Immutable staff-toolkit-original-browser.json records original populated tables, fields, widths and all contract comparison modes.
- Preserve all cost math, table values, price comparisons, dates, labels, IDs, field contracts, native downloads and API payloads. Volume GET /api/quote-sequence/VQ allocates a number and must be mocked. Blog preview POST is synthetic; all publishing, image uploads, portal/reward mutations, mailing/Mailchimp and product writes must stay mocked. Unknown APIs/fetch/xhr fail closed.
- Adopt a scoped owner and canonical components after baseline review. Audit every old CSS consumer, including tests/ui/test-jim-mailing-list.html and its stub. art-hub.css, dash-shell.css and blog.css still have other consumers and must not be deleted. Review loading, empty, error, retry, stale-response, dialog, mobile, keyboard, export and print states before crediting migration.
- Finish isolated review, record an exact reviewed SHA, integrate only that SHA with current primary memory, run required local gates and actual exact-source CI, then make one verified release. Never merge an advancing worktree blindly.
...
### Current isolated draft — three toolkit pages

Contract Break-Even, Roland Supplies and Volume Quote use the draft staff-toolkit.css owner with canonical tokens/components, Public Sans and navy/gold. All three direct CSS graphs are 83,637 LF bytes; originals were 166,499 / 164,993 / 173,304. Zero flags and undefined tokens. No page-specific CSS retired, no migration credit yet; five other toolkit pages remain unchanged.
All 31 source contracts pass with 51 reversible HTML/controller mappings. The earlier pair passed 258 source/CSS guards. All 22 focused browser scenarios pass: four-width layouts/axe, original/current contract and volume calculations, unchanged payloads, loading/error/retry, late pricing/stock responses, removed/cleared rows, duplicate pending saves, malformed sequence responses and uncertain/partial save messaging. Controller ESLint is clean; e2e fixtures/specs are outside the existing lint scope.
Two contract PDFs / six pages were reviewed. Volume memo and customer PDFs each remain one page, were visually reviewed, and extracted content matches both immutable original PDFs exactly after whitespace normalization. Customer print excludes the internal memo. All allocating GETs, saves and other business writes remain mocked. Roland provider UI remains a separate mocked boundary; original embed and printable-PDF links are preserved.
Volume controls use canonical fields/buttons and keyboard-scrollable result tables. The phone memo is bounded and keyboard-scrollable, with full content retained on paper; phone KPI cards remain legible in two columns. Proven old-response bugs were fixed: earlier garment pricing and stock responses can no longer replace the latest selection or mutate a cleared row. Pricing/stock retry stays in the affected row. Pending saves hold controls and use captured quote values; invalid quote numbers stop before posting; partial/uncertain saves direct staff to check Quote Management before retrying. The original price/cost algorithms and successful request bodies are unchanged.
Next: Product Manager, then Blog Editor, Portal Admin, Jim Mailing List and Past Due Orders. Capture original edit/request/print states using immutable source reconstruction, preserve vendor-code parity and every API payload, then migrate/review each page. Audit all old CSS consumers and the Jim preview/stub before retiring anything. Full application gates and exact-source CI are still required before deployment.
