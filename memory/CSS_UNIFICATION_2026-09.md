...
Erik resumed; standing edit/deploy authorization continues. **Frontend v2026.09.08.15 / Heroku 2074 is LIVE**, full SHA **6652db8b8ffd6a8cde436b637a1e8f3442437b99**, actual slug verified 2026-09-09T03:33:42Z. Exact-source CI **34306633189** passed all four jobs, including live money/calculator parity, on **20d3e247ee8519e99cb51084d9099467cf78588e**. Eight deployed runtime assets match committed bytes; four manual HTML shells and two public form regression shells return 200; four staff shells remain protected. No business writes or notifications. Main/tag/develop were synchronized; subsequent documentation commits are expected. Track follow-up main/develop CI separately.

**56 reviewed application pages live; 169 pending**, from **225 application/served-archive HTML sources**. Another 79 tracked HTML sources are excluded. **22 of 26 tracked training pages are reviewed; four remain.** Ignored local training/api-test-runner.html is not tracked/deployed and is excluded; leave it alone. Generated document/style owners remain separate unfinished work.

### Next batch and plan

- [x] Unify/deploy customer-service, get-to-know-erik, sales-coordinator-manual and sales-coordinator-training-schedule (.15).
- [x] Preserve original prose, lessons, staff data, figures and media; verify all 44 manual and 12 schedule sections, biography disclosure, roster dates, exercise feedback, keyboard navigation and complete printing.
- [ ] Finish training/index, sales-tax-code-trainer, shopworks-customer-setup and shopworks-customer-setup-enhanced in **separate worktree C:/Users/erik/.codex/visualizations/2026/09/07/01a07d90-9a4c-7e70-9e4e-c196377b7c6b/training-final-worktree**, branch **codex/training-final-family**, base 20d3e247. It has uncommitted work belonging to this task; do not delete/reset it. node_modules is a junction to the primary frontend. No .env copied.
- [ ] Last-four layout implemented locally; basic setup/tax trainer control fixes started. Enhanced simulator and Training Center retry handling still need implementation; all four need focused browser, content and visual verification. No claim that this worktree is ready/live.
- [ ] Integrate the reviewed worktree into current develop, preserving .15 release/documentation changes; full release gates, exact-source CI and deployment, then remaining application families.

Design: Public Sans, white/neutral/forest tokens, shared controls and reading rhythm. Last-four work uses scoped training-simulator.css plus existing components/training-practice. Preserve lesson data and simulator fields. Repair keyboard access, phone overflow, duplicate restart listeners, grading the current speed answer, round/timer cancellation and unavailable/malformed saved progress. Training Center must keep curated tracks usable and show/retry live-list failures.

### Verification / resume evidence

.15: **207 unit suites / 5,054 passed / four existing skips**, 88 DOM, four a11y unit, 84 quote parity; **192 mocked browser plus 16 remaining browser cases**, three optional screenshot skips, all five calculator surfaces. Build/lint/types/**293 CSS**/HTTP boot pass; 485 routes / 24 modules unchanged. Fourteen focused manual browser cases and 109 ownership/content checks pass. PDFs reviewed: customer service 54 pages, biography 2, manual 63, schedule 34; **all 2,910 selected paper-content blocks present**. Unique family CSS source reduced 52% (not network transfer).

Artifacts: C:/Users/erik/.codex/visualizations/2026/09/07/01a07d90-9a4c-7e70-9e4e-c196377b7c6b. training-manual-{release-record,source-ci,heroku-verification,live-verification,local-gates}.json are authoritative. training-final-baseline-record.json records the next four originals. Do not rerun one-shot installers/release helpers. New Heroku push helper is single attempt: inspect actual release SHA/status after a disconnect before retrying (a prior automatic retry built the same SHA twice).
