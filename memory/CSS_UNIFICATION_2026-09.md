...
Standing edit/deploy authorization continues. **Frontend v2026.09.08.20 / Heroku 2079 is LIVE**, full SHA **8d624d470346314c5dffa209a8f72598a4d6db83**, actual slug verified 2026-09-09T08:50:19.6072095Z. Exact-source CI **34330122846** passed all four jobs including actual live money/calculator parity on **a71e4e24ee620f1c82347ad24d14728b6829d744**. Both deployed webstore runtime assets match committed bytes; all twelve page sources and twelve public route aliases serve the unified version. Five regression staff routes remain gated and ten prior public/reference/form shells return200. No business writes or notifications.

**84 reviewed application pages live;141 pending**, from225 application/served-archive HTML sources. Another79 tracked HTML sources are excluded (12 archive,14 email,50 fixtures/templates,3 retired). All26 training pages are reviewed/live. Generated document/style owners remain separate unfinished work. Ignored training/api-test-runner.html is not tracked/deployed; leave it alone.

### Next batch and plan

- [x] Twelve public webstore guides deployed after the policy CMS (.19), references (.18) and API references (.17). Shared styling, native navigation/FAQs, original text/SEO/imagery and773 paper content blocks verified.
- [x] Fifteen brand guides reviewed and committed: branch **codex/brand-guide-family**, commit **a6d59feca31b1a4419c1f963bb91c063e7d03539**, base **b417e03de7f9db0aea8bd6f485b2c2c947bd207b**.31 focused browser cases pass,168 content/ownership/runtime checks; all1261 PDF text/product/FAQ blocks retained. Native menu includes keyboard wrap/return, Escape/backdrop/close and desktop resize. All fifteen search/route aliases and four widths/axe verified. Raw CSS graph65,000 ->72788 bytes. Shared owners replace the former Carhartt-named files. NOT deployed.
- [ ] Integrate only the reviewed brand branch into primary, run full local checks and exact-source CI, then verify actual release. Candidate2026.09.09.1 is pre-versioned on its fifteen HTML files. After verified rollout:99 reviewed/126 pending.
- [x] Next five staff references planned from brand commit: commission-structure, embroidery-bonus-plan, seo-strategy, forms-library, data-entry-guide. Current isolated branch **codex/staff-reference-family** contains plan/baseline only for these five; no app edits yet. Its working directory is **C:/Users/erik/.codex/visualizations/2026/09/07/01a07d90-9a4c-7e70-9e4e-c196377b7c6b/training-final-worktree**. node_modules is a junction; no .env copied.
- [ ] Continue staff references, remaining storefront/staff/calculator/builder families and generated document owners.

### Verification / resume evidence

.20:212 unit suites/5,122 passed/four existing skips;88 DOM,4 accessibility unit,84 quote parity;290 mocked browser in486.5s;16 remaining browser in4.2min,3 optional screenshot skips. All5 live calculator surfaces including EMB/CAP additional-logo tiers. Build/lint/types/295 CSS/HTTP boot pass;485 routes/24 modules unchanged. The earlier interrupted browser run was resumed and passed before CI; never count the interrupted run as a pass.

Artifacts: **C:/Users/erik/.codex/visualizations/2026/09/07/01a07d90-9a4c-7e70-9e4e-c196377b7c6b**. webstore-{release-record,source-ci,heroku-verification,live-verification,local-gates}.json are authoritative. brand-guide-{worktree-record,owned-files,original-baseline,css-measurement,paper-review}.json hold the reviewed next batch. brand-guide-isolated.config.cjs uses3421. staff-reference-original-baseline.json holds the next five originals. Do not rerun one-shot installers/finalizers/release helpers. After an uncertain push inspect actual Heroku SHA before retrying.
