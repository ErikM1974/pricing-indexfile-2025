...
- [x] Fix the three initial phone issues and install/register the 35 runtime files, ownership/content guards and focused browser spec locally.
- [ ] Complete interaction and print verification. First focused run: 34 passed, three failed (credit-card and name-personalization extra print pages, missing customer listbox name). First ownership run: 68 passed, one undefined-token failure. Fixes are being checked; forms are not released. Remaining checks include drafts, style/swatch menus, money/provenance and filled print; mock every business service/write.
...
The first draft was checked at 1440/768/390/320 and printed with real images. The three original phone issues are fixed. Installed-source print checks uncovered two page-count regressions missed by the earlier draft preview; fixes and populated-state print still need verification. Templates retain their original legal/HR/payment prose and examples; this CSS work does not review or change those policies.
