...
(function() {
            ...
            document.addEventListener('keydown', function(e) {
                ...
                if (e.key === 'Escape' && alModal.classList.contains('active')) {
            ...
            var GARMENT_AL = [
                { qty: '1-7 pcs', price: 10.00, ltm: true },
                { qty: '8-23 pcs', price: 9.00 },
                { qty: '24-47 pcs', price: 8.00 },
                { qty: '48-71 pcs', price: 7.50 },
                { qty: '72+ pcs', price: 7.00 }
            ];
            var CAP_AL = [
                { qty: '1-7 pcs', price: 6.50, ltm: true },
                { qty: '8-23 pcs', price: 5.50 },
                { qty: '24-47 pcs', price: 4.75 },
                { qty: '48-71 pcs', price: 4.50 },
                { qty: '72+ pcs', price: 4.25 }
            ];

            function openALModal(stitchCount, designNum) {
                document.getElementById('al-design-label').textContent = '#' + designNum;

                var garmentBase = 8000;
                var capBase = 5000;
                var garmentRate = 1.25;
                var capRate = 1.00;
                ...
                if (garmentOverage > 0) {
                    infoHtml += 'Garment AL: <span class="overage-amount">+$' + garmentOverage.toFixed(2) + '/piece overage</span> (' + garmentOverK + 'K over 8K base). ';
                } else {
                    infoHtml += 'Garment AL: <span class="within-base">within 8K base</span>. ';
                }
                if (capOverage > 0) {
                    infoHtml += 'Cap AL: <span class="overage-amount">+$' + capOverage.toFixed(2) + '/piece overage</span> (' + capOverK + 'K over 5K base).';
                } else {
                    infoHtml += 'Cap AL: <span class="within-base">within 5K base</span>.';
                }
                infoEl.innerHTML = infoHtml;

                /* Populate tables */
                populateALTable('al-garment-table', GARMENT_AL, garmentOverage);
                populateALTable('al-cap-table', CAP_AL, capOverage);

                /* Show/hide overage column */
                var garmentCols = document.querySelectorAll('#al-garment-table .al-total-col');
                var capCols = document.querySelectorAll('#al-cap-table .al-total-col');
                garmentCols.forEach(function(el) { el.style.display = garmentOverage > 0 ? '' : 'none'; });
                capCols.forEach(function(el) { el.style.display = capOverage > 0 ? '' : 'none'; });
            ...
            function populateALTable(tableId, tiers, overage) {
                ...
                tiers.forEach(function(t) {
                    ...
                    if (t.ltm) html += ' <span class="ltm-tag">+$50 LTM</span>';
                    html += '</td>';
            ...
            alModal.addEventListener('click', function(e) {
                if (e.target === alModal || e.target.closest('.al-modal-close')) {
                    alModal.classList.remove('active');
                }
            });
            ...
            function buildCard(fields, dl) {
                ...
                if (stitchTier === 'Full Back') {
                    html += '<div class="as-surcharge-badge warn">Full Back LTM Fee: $100.00 (orders of 1-23 pieces)</div>';
                } else if (asVal > 0) {
                ...
                if (stitchTier !== 'Full Back') {
                    ...
                    html += '<button class="al-pricing-btn" type="button" data-stitches="' + stitchNum + '" data-design="' + escapeHtml(designNum) + '">AL Pricing</button>';
                }
