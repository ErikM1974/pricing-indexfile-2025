...
async function open(page,state={}){
 ...
 await page.context().route('**/*',async route=>{
  ...
  // Live-like rules (2026-09-16): Caps allow no garment method; Personal Protection allows all but DTG.
  // Workwear keeps its old embroidery-only row: Quick Quote must still price screen print and DTF on it.
  if(p==='/api/decoration-methods')return route.fulfill({json:{rules:[{category:'T-Shirts',EMB:true,DTG:true,SCP:true,DTF:true},{category:'Caps',EMB:false,DTG:false,SCP:false,DTF:false},{category:'Workwear',EMB:true,DTG:false,SCP:false,DTF:false},{category:'Personal Protection',EMB:true,DTG:false,SCP:true,DTF:true}],overrides:[]}});
