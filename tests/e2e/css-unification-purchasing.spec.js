...
for (const name of ['purchasing-portal', 'ae-mission-control']) test('CSS purchasing: ' + name + ' invoice ignores an older lookup and preserves viewer mode', async ({ page }) => {
    ...
    expect(await page.locator('#smiv-modal').evaluate(n => n.tagName)).toBe(name === 'ae-mission-control' ? 'DIV' : 'DIALOG'); await page.keyboard.press('Escape'); await expect(page.locator('#smiv-modal')).toBeHidden(); clean(events);
});
