...
function fixture(name,record){const file='tests/fixtures/contract-calculators-'+name+'-original-browser.json',abs=path.join(root,file);
 ...
 else expect(record).toEqual(JSON.parse(fs.readFileSync(abs,'utf8')));
}
async function evidence(page,name,e){const states=[];fs.mkdirSync(out,{recursive:true});
 ...
 else {const old=JSON.parse(fs.readFileSync(path.join(root,file),'utf8'));for(let i=0;i<states.length;i++)for(const k of ['title','url','ids','fields','links','tables','headings'])expect(states[i][k],name+' '+k).toEqual(old.states[i][k]);expect(e.dialogs).toEqual(old.dialogs);}
 await page.setViewportSize({width:1440,height:1000});await page.pdf({path:path.join(out,'contract-calculators-'+name+'-'+phase+'.pdf'),format:'Letter',printBackground:true});
}
