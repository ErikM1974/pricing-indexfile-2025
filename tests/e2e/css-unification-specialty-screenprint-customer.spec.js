...
for(const quantity of [24,145])test('CSS specialty customer-supplied screen print: saved invoice '+quantity,async({page})=>{
 ...
 await page.evaluate(()=>{window.__invoiceHTML='';window.NWCAStaffPrint={printWhenReady:async()=>{}};window.open=()=>({document:{write:html=>{window.__invoiceHTML=html;},close:()=>{}},addEventListener:()=>{}});});
 ...
 record('invoice-'+quantity,{title:await popup.title(),text:await popup.locator('body').innerText(),tables:await popup.locator('table').allTextContents()});
 for(const width of [1440,768,390,320]){
  await popup.setViewportSize({width,height:1000});await assets(popup);
  if(!original){expect(await popup.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1)).toBe(true);expect((await new AxeBuilder({page:popup}).withTags(['wcag2a','wcag2aa','wcag21aa']).analyze()).violations).toEqual([]);}
  await popup.screenshot({path:path.join(out,'customer-invoice-'+quantity+'-'+phase+'-'+width+'.png'),fullPage:true});
  if(!original){const scroll=popup.locator('.invoice-table-scroll');const max=await scroll.evaluate(n=>n.scrollWidth-n.clientWidth);if(max>1){await scroll.focus();await popup.keyboard.press('ArrowRight');await expect.poll(()=>scroll.evaluate(n=>n.scrollLeft)).toBeGreaterThan(0);const step=await scroll.evaluate(n=>n.clientWidth);for(let x=Math.min(step,max);;x=Math.min(x+step,max)){await scroll.evaluate((n,x)=>{n.scrollLeft=x;},x);await popup.screenshot({path:path.join(out,'customer-invoice-'+quantity+'-horizontal-x'+x+'-'+phase+'-'+width+'.png'),fullPage:true});if(x===max)break;}await scroll.evaluate(n=>{n.scrollLeft=0;});}}
 }
 await popup.pdf({path:path.join(out,'customer-invoice-'+quantity+'-'+phase+'.pdf'),preferCSSPageSize:true,format:'Letter',printBackground:true});check(expect,events);await popup.close();
});
