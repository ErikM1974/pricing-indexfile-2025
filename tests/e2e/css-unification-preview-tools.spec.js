...
test('CSS preview tools: embroidery thread picker supports keyboard search, choice and return', async ({ page }) => {
    ...
    await trigger.click(); await page.locator('#pickerSearch').fill('white'); await expect(page.locator('.picker-cell .picker-name').first()).toHaveText(/white/i); const chosen = await page.locator('.picker-cell .picker-name').first().innerText(); await page.locator('.picker-cell').first().click(); await expect(page.locator('#pickerOverlay')).toBeHidden(); await expect(page.locator('.thread-name').first()).toHaveText(chosen); await expect(page.locator('.thread-swatch').first()).toBeFocused(); expect(events).toEqual({ errors: [], writes: [], unmocked: [] });
});
