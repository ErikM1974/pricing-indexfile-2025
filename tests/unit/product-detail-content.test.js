...
const fixture = require('../fixtures/product-detail-original-content.json');
const root = path.resolve(__dirname, '../..'), norm = s => s.replace(/\s+/g, ' ').trim();
test.each(fixture.pages)('$file preserves product instructions, destinations and original inputs', original => {
    ...
    expect([...d.querySelectorAll('[id]')].map(n => n.id).filter(id => id !== 'product-detail-main').sort()).toEqual([...original.ids].sort());
...
test.each(Object.keys(fixture.hashes))('%s preserves original product/API/stock/export helpers outside recorded UI edits', file => {
