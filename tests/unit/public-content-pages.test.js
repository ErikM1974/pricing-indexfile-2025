...
describe('content pages — specifics', () => {
    ...
    test('laser tumbler: spinner/error via hidden attribute', () => {
        ...
        expect(html).toMatch(/<div id="loading-spinner" class="tumbler-loading" role="status">/);
        expect(html).toMatch(/<p id="error-message" role="alert" hidden><\/p>/);
        ...
        expect(read('shared_components/css/specialty-pricing.css')).toContain('text-align: right');
        expect(read('shared_components/css/components.css')).toMatch(/\[hidden\]\s*\{[^}]*display:\s*none;/);
