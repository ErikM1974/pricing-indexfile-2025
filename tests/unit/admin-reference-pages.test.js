...
describe('admin + reference pages — shared hygiene', () => {
    test.each(PAGES)('%s', (name) => {
        ...
        if (html.includes('data-ui="unified"')) {
            ...
            const owner = ['access-admin', 'drive-access'].includes(name) ? 'shared_components/css/staff-admin-tools.css' : ['api-usage', 'table-usage-audit', 'bandit-integration'].includes(name) ? 'shared_components/css/staff-monitoring.css' : `dashboards/css/${name}.css`;
