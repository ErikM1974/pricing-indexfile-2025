...
describe('workable', () => {
    ...
    test('the stat tiles are buttons that filter', () => {
        ...
        expect(css).toContain('.pp-tile.is-active');
    });
    test('rows link to their JotForm submission and invoice buttons are labelled', () => {
        ...
        expect(js).toMatch(/class="pp-req-link btn btn-secondary"/);
    ...
    test('status chips use the state tokens, not hex', () => {
        const chips = css;
        expect(chips).toMatch(/\.pp-chip--ordered[^}]*var\(--color-info-/);
...
describe('phone + freshness', () => {
    test('low-value columns hide under 640px', () => {
        ...
        expect(css).toMatch(/@media screen and \(width <= 640px\)[\s\S]*\.pp-col-phone-hide \{ display: none; \}/);
    });
