...
const fixture = require('../fixtures/staff-monitoring-original-content.json');
...
test.each(fixture.pages)('$file preserves source content and dependencies', original => {
    ...
    expect([...doc.querySelectorAll('[id]')].map(n => n.id).filter(id => !id.startsWith('monitor-'))).toEqual(original.ids);
...
test.each(Object.keys(fixture.hashes))('%s changes only recorded presentation and error checks', file => {
    ...
    for (const change of fixture.changes.filter(c => c.file === file).reverse()) {
        expect(source).toContain(change.to); source = source.split(change.to).join(change.from);
    }
