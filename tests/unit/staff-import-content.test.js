...
const fixture = require('../fixtures/staff-import-original-content.json');
...
test.each(fixture.pages)('$file preserves source content and dependencies', original => {
    ...
    expect([...doc.querySelectorAll('[id]')].map(n => n.id).filter(id => !id.startsWith('import-'))).toEqual(original.ids);
