...
// Most handlers retain the original body; paid storefront payload stages now live in lib/storefront-order.
// (tests/unit/server-route-table.test.js). Everything it needs from server.js arrives in ctx; nothing is global.
const { buildStorefrontPayload } = require('../lib/storefront-order/payload');
...
app.post('/api/submit-3day-order', async (req, res) => {
  try {
    ...
    const manageOrdersPayload = buildStorefrontPayload({ tempOrderNumber, customerData, colorConfigs, orderTotals, pricingData, orderSettings, paymentConfirmed, stripeSessionId, paymentAmount }, { channelConfig, nowPacificNaiveIso });
