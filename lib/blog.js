...
// xss ships ESM-style typings (`export default`), so under CommonJS require()
// tsc sees the module namespace and calls it non-callable. Runtime is fine —
// the package assigns module.exports = filterXSS — so restate the call signature.
/** @type {(html: string, options?: object) => string} */
const xss = /** @type {any} */ (require('xss'));
...
function renderMarkdown(markdown) {
  ...
  // marked.parse is typed `string | Promise<string>` (it only goes async with
  // async extensions, which this renderer doesn't use) — pin the sync branch.
  let html = xss(/** @type {string} */ (marked.parse(withTokens)), XSS_OPTIONS);
...
// ---------- style → posts map (product-page "From our blog" block) ----------

// Derived live from published post bodies (every /product.html?style=X link),
// so a new post wires itself into its product pages within one cache window —
// no deploy, no generated file. Consumed by GET /api/blog-product-map, rendered
// client-side by product/js/related-posts.js.
async function productMap() {
  return cached('product-map', async () => {
    const posts = (await listPublished()).slice(0, 200);
    const map = {};
    for (const p of posts) {
      const full = await getPublished(p.slug).catch(() => null);
      if (!full) continue;
      const styles = new Set();
      const re = /\/product\.html\?style=([A-Za-z0-9._/-]{2,40})/g;
      let m;
      while ((m = re.exec(String(full.bodyMarkdown || '')))) styles.add(m[1].toUpperCase());
      for (const s of styles) (map[s] = map[s] || []).push({ slug: p.slug, title: p.title });
    }
    return map;
  });
}
...
module.exports = {
  ...
  readingMinutes, fmtDate, escapeHtml, CACHE_TTL_MS, productMap,
};
