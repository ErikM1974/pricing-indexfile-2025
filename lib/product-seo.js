...
// Per-style head cache. 10 min → 60 min on 2026-09-07: the head now comes from
// the proxy's whole-catalog map (/api/product-heads/:style, one Caspio query per
// 24 h for all ~4,500 styles), so a repeat fetch costs nothing at Caspio either
// way — the longer TTL just spares the proxy a round trip per crawled page.
// Product_Copy edits still land within the proxy's own 10-min overlay window
// plus this hour.
...
async function headForStyle(styleRaw) {
  ...
  // /api/product-heads/:style (2026-09-07) replaces /api/product-details here.
  // Search-engine crawlers (Semrush, Amazonbot, Googlebot, bingbot) walk every
  // product URL overnight at ~50 pages an hour, and each page view was a full
  // every-colour Sanmar_Bulk read at Caspio — ~1,000–1,500 calls a day, 100% for
  // bots. The head record is the same fields buildHead() reads (title, brand,
  // description, category, images) served from the proxy's 24 h catalog map, and
  // it already folds in Product_Copy overrides and rep-added non-SanMar styles.
  // A 404 is a genuine unknown style: NO fallback to product-details — that
  // would hand every junk ?style= a crawler invents its own Caspio read.
  let head = null;
  try {
    const row = await fetchJson(
      `${CASPIO_PROXY_BASE}/api/product-heads/${encodeURIComponent(style)}`, 4000);
    if (row && row.STYLE) head = buildHead(style, row);
  } catch (e) {
    if (e.message !== 'HTTP 404') console.error(`[product-seo] head fetch failed for ${style}:`, e.message);
  }
