...
module.exports = function createHolidayService(ctx) {
    ...
    function staffInvitation() {
        const campaign = readCampaign();
        const details = { closesAt: campaign.closesAt, deadlineLabel: campaign.deadlineLabel };
        if (now() >= Date.parse(campaign.closesAt)) return { ...details, closed: true };
        const code = (ctx.giftCode || process.env.CHRISTMAS_GIFT_CODE || '').trim();
        // Display only a configured code that the redemption endpoint actually accepts.
        try {
            validateCode(code);
        } catch {
            throw problem('The invitation code is unavailable. Contact Erik or retry.', 503);
        }
        return { ...details, closed: false, code };
    }
    ...
    return { readCampaign, product, validateCode, staffInvitation, estimate, submit };
};
