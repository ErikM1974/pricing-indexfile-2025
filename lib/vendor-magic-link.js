...
// ── Permanent access link (Erik's "no magic link" request, 2026-07-20) ──────
// One long-lived signed URL the vendor bookmarks: /vendor/access/<token>.
// Clicking it re-checks the LIVE Enabled flag and then sets the normal 30-day
// session cookie — so the link is effectively permanent for an enabled vendor,
// and dies within ~60s of Erik flipping Enabled='No' in Caspio. Distinct type
// tag ('vaccess') so it can never be replayed as a session or emailed link.
const ACCESS_TTL_SEC = 5 * 365 * 24 * 60 * 60; // ~5 years

function mintAccessToken({ email, vendorName }) {
  if (!linkSecret()) return null;
  return signPayload({
    e: String(email).toLowerCase().trim(),
    v: String(vendorName),
    x: Math.floor(Date.now() / 1000) + ACCESS_TTL_SEC,
    t: 'vaccess',
  }, linkSecret());
}
// Throws if invalid/expired/tampered.
function verifyAccessToken(token) {
  if (!linkSecret()) throw new Error('not configured');
  const obj = verifySigned(token, linkSecret());
  if (obj.t !== 'vaccess' || !obj.e || !obj.v) throw new Error('incomplete');
  return { email: obj.e, vendorName: String(obj.v) };
}
...
// ── Proof-image capability token (2026-08-06) ───────────────────────────────
// Same problem the customer portal hit, one identity over: transfer mockup
// thumbnails are stored as the proxy's /api/box/thumbnail/<fileId>, which is
// requireStaff — and a VENDOR session is not a staff session, so every mockup in
// the Supacolor / L&P portal 401s. Repointing the url at this origin (boxUrl)
// does nothing here: same-origin still lands on requireStaff.
//
// So: a capability, minted ONLY while projecting a row that vendorOwnsRow() has
// already confirmed belongs to this vendor. The vendor never sees or supplies a
// Box file id, so there is nothing to enumerate.
//
// 🔴 Distinct type tag ('vproof') and signed with the SESSION secret. Two token
// types under one key with no discriminator is how a stolen image capability
// becomes a session — the customer portal's 'proof' tag exists for exactly this
// reason, and these two must never verify as each other.
const PROOF_TTL_SEC = 12 * 60 * 60;

function mintProofToken({ fileId, vendorName }) {
  if (!sessionSecret()) return null;
...
// Returns { fileId, vendorName } or null (never throws — a bad token is just a 404).
function verifyProofToken(token) {
  if (!sessionSecret()) return null;
  try {
    const obj = verifySigned(token, sessionSecret());
    if (obj.t !== 'vproof' || !obj.f || !obj.v) return null;
    if (!/^\d+$/.test(String(obj.f))) return null;
    return { fileId: String(obj.f), vendorName: String(obj.v) };
  } catch { return null; }
}
